...
public interface SupplierBillRepository extends JpaRepository<SupplierBill, Long> {
  ...
  @Query(
      "SELECT b FROM SupplierBill b WHERE b.company = :company AND b.contact = :contact "
          + "AND b.status = 'POSTED' AND (b.total - b.amountPaid) > 0 ORDER BY b.dueDate ASC")
  List<SupplierBill> findOutstandingByCompanyAndContact(
      @Param("company") Company company, @Param("contact") Contact contact);

  // Total outstanding AP balance
  @Query(
      "SELECT COALESCE(SUM(b.total - b.amountPaid), 0) FROM SupplierBill b "
  ...
  // Bills by date range
  @Query(
      "SELECT b FROM SupplierBill b WHERE b.company = :company "
          + "AND b.billDate >= :startDate AND b.billDate <= :endDate "
          + "ORDER BY b.billDate DESC, b.billNumber DESC")
  List<SupplierBill> findByCompanyAndDateRange(
  ...
  List<SupplierBill> searchByCompany(

...
public interface UserSecurityLevelRepository extends JpaRepository<UserSecurityLevel, Long> {
  ...
  @Query(
      "SELECT usl FROM UserSecurityLevel usl WHERE usl.user.id = :userId AND usl.company.id = :companyId")
  Optional<UserSecurityLevel> findByUserIdAndCompanyId(
  ...
  @Query(
      "SELECT COALESCE(usl.maxLevel, 0) FROM UserSecurityLevel usl WHERE usl.user.id = :userId AND usl.company.id = :companyId")
  Optional<Integer> getMaxLevelByUserIdAndCompanyId(
      @Param("userId") Long userId, @Param("companyId") Long companyId);

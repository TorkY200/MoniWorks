...
public interface UserInvitationRepository extends JpaRepository<UserInvitation, Long> {
  ...
  @Query(
      "SELECT COUNT(i) > 0 FROM UserInvitation i WHERE i.email = :email AND i.company = :company AND i.status = 'PENDING' AND i.expiresAt > :now")
  boolean existsPendingInvitation(
      @Param("email") String email, @Param("company") Company company, @Param("now") Instant now);
  ...
  @Query(
      "SELECT COUNT(i) FROM UserInvitation i WHERE i.company = :company AND i.status = 'PENDING' AND i.expiresAt > :now")

...
public interface KPIValueRepository extends JpaRepository<KPIValue, Long> {
  ...
  @Query(
      "SELECT kv FROM KPIValue kv WHERE kv.kpi = :kpi "
          + "AND kv.period.fiscalYear.id = :fiscalYearId ORDER BY kv.period.startDate")
  List<KPIValue> findByKpiAndFiscalYear(
      @Param("kpi") KPI kpi, @Param("fiscalYearId") Long fiscalYearId);

  @Query(
      "SELECT kv FROM KPIValue kv WHERE kv.kpi.company.id = :companyId "
          + "AND kv.period.fiscalYear.id = :fiscalYearId ORDER BY kv.kpi.code, kv.period.startDate")
  List<KPIValue> findByCompanyAndFiscalYear(
      @Param("companyId") Long companyId, @Param("fiscalYearId") Long fiscalYearId);

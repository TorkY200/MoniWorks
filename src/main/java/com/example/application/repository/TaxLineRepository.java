...
/**
 * Repository for TaxLine entities. Provides queries for tax reporting and GST return generation.
 */
@Repository
public interface TaxLineRepository extends JpaRepository<TaxLine, Long> {
  ...
  @Query(
      "SELECT t FROM TaxLine t WHERE t.company = :company "
  ...
  /**
   * Sum tax amounts by tax code for a date range. Returns [taxCode, sumTaxableAmount, sumTaxAmount]
   */
  @Query(
      "SELECT t.taxCode, SUM(t.taxableAmount), SUM(t.taxAmount) FROM TaxLine t "
  ...
  /**
   * Sum tax amounts by report box for a date range. Returns [reportBox, sumTaxableAmount,
   * sumTaxAmount]
   */
  @Query(
      "SELECT t.reportBox, SUM(t.taxableAmount), SUM(t.taxAmount) FROM TaxLine t "
          + "WHERE t.company = :company "
          + "AND t.entryDate >= :startDate AND t.entryDate <= :endDate "
  ...
  @Query(
      "SELECT COALESCE(SUM(t.taxableAmount), 0) FROM TaxLine t "
          + "WHERE t.company = :company "
          + "AND t.entryDate >= :startDate AND t.entryDate <= :endDate")
  ...
  @Query(
      "SELECT COALESCE(SUM(t.taxAmount), 0) FROM TaxLine t "
          + "WHERE t.company = :company "
          + "AND t.entryDate >= :startDate AND t.entryDate <= :endDate")

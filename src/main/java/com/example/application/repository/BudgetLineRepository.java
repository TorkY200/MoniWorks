...
public interface BudgetLineRepository extends JpaRepository<BudgetLine, Long> {
    ...
    @Query("SELECT bl FROM BudgetLine bl WHERE bl.budget = :budget " +
           "AND bl.period.fiscalYear.id = :fiscalYearId ORDER BY bl.period.startDate, bl.account.code")
    List<BudgetLine> findByBudgetAndFiscalYear(@Param("budget") Budget budget,
                                                @Param("fiscalYearId") Long fiscalYearId);

    @Query("SELECT SUM(bl.amount) FROM BudgetLine bl WHERE bl.budget = :budget " +
           "AND bl.account = :account AND bl.period.fiscalYear.id = :fiscalYearId")
    BigDecimal sumByBudgetAndAccountAndFiscalYear(@Param("budget") Budget budget,

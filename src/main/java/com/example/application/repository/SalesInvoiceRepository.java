...
public interface SalesInvoiceRepository extends JpaRepository<SalesInvoice, Long> {
  ...
  List<SalesInvoice> findOverdueByCompany(
      @Param("company") Company company, @Param("today") LocalDate today);
  ...
  @Query(
      "SELECT i FROM SalesInvoice i WHERE i.company = :company AND i.contact = :contact "
          + "AND i.status = 'ISSUED' AND (i.total - i.amountPaid) > 0 ORDER BY i.dueDate ASC")
  List<SalesInvoice> findOutstandingByCompanyAndContact(
      @Param("company") Company company, @Param("contact") Contact contact);

  // Total outstanding AR balance
  @Query(
      "SELECT COALESCE(SUM(i.total - i.amountPaid), 0) FROM SalesInvoice i "
  ...
  BigDecimal sumOverdueByCompany(
      @Param("company") Company company, @Param("today") LocalDate today);

  // Invoices by date range
  @Query(
      "SELECT i FROM SalesInvoice i WHERE i.company = :company "
          + "AND i.issueDate >= :startDate AND i.issueDate <= :endDate "
          + "ORDER BY i.issueDate DESC, i.invoiceNumber DESC")
  List<SalesInvoice> findByCompanyAndDateRange(
  ...
  List<SalesInvoice> searchByCompany(
      @Param("company") Company company, @Param("search") String search);
  ...
  @Query(
      "SELECT i FROM SalesInvoice i WHERE i.company = :company AND i.type = 'INVOICE' "
          + "ORDER BY i.issueDate DESC, i.invoiceNumber DESC")
  List<SalesInvoice> findInvoicesByCompany(@Param("company") Company company);

  // Find only credit notes for a company
  @Query(
      "SELECT i FROM SalesInvoice i WHERE i.company = :company AND i.type = 'CREDIT_NOTE' "
          + "ORDER BY i.issueDate DESC, i.invoiceNumber DESC")
  ...
  List<SalesInvoice> findByCompanyAndContactAndDateRange(
      @Param("company") Company company,
  ...
  BigDecimal sumOutstandingByContactAsOfDate(
      @Param("company") Company company,
      @Param("contact") Contact contact,

...
/**
 * Component for customizing grid column visibility and ordering. Integrates with SavedViewService
...
public class GridCustomizer<T> extends HorizontalLayout {
  ...
  public record ColumnConfig(String key, String header, boolean visible, int order) {}
  ...
  private void loadDefaultView() {
    if (company != null && user != null) {
      savedViewService
          .findDefaultView(company, user, entityType)
          .ifPresent(
              view -> {
  ...
  public void applyView(SavedView view) {
    ...
    try {
      List<ColumnConfig> configs =
          objectMapper.readValue(view.getColumnsJson(), new TypeReference<List<ColumnConfig>>() {});

      // Build a map of column positions
      Map<String, ColumnConfig> configMap =
          configs.stream().collect(Collectors.toMap(ColumnConfig::key, c -> c));
      ...
      sortedColumns.sort(
          (a, b) -> {
            int orderA =
                configMap.containsKey(a.getKey()) ? configMap.get(a.getKey()).order() : 999;
            int orderB =
                configMap.containsKey(b.getKey()) ? configMap.get(b.getKey()).order() : 999;
  ...
  private void openColumnDialog() {
    ...
    for (var column : grid.getColumns()) {
      ...
      if (key != null) {
        String header =
            originalColumns.containsKey(key)
  ...
  private void resetColumnOrder() {
    ...
    sortedColumns.sort(
        (a, b) -> {
          int orderA =
              originalColumns.containsKey(a.getKey())
                  ? originalColumns.get(a.getKey()).order()
                  : 999;
          int orderB =
              originalColumns.containsKey(b.getKey())
                  ? originalColumns.get(b.getKey()).order()
                  : 999;
          return Integer.compare(orderA, orderB);
        });
    grid.setColumnOrder(sortedColumns);
  ...
  private void saveCurrentView() {
    ...
    try {
      ...
      Notification.show(
              "Failed to save view: " + e.getMessage(), 3000, Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_ERROR);
    }
  }
        ...
        new Button(
            ...
            e -> {
              ...
              try {
                ...
                Notification.show(
                        "View \"" + name + "\" saved", 2000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                ...
                Notification.show(
                        "Failed to save view: " + ex.getMessage(),
                        3000,
    ...
    if (views.isEmpty()) {
      ...
      for (SavedView view : views) {
        ...
        if (view.isDefault()) {
          ...
          setDefaultBtn.addClickListener(
              ev -> {
        ...
        deleteBtn.addClickListener(
            ev -> {
              ...
              if (viewSelector.getValue() != null
                  && viewSelector.getValue().getId().equals(view.getId())) {
                viewSelector.clear();
              }
              dialog.close();
              openManageViewsDialog();

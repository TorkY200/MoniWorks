...
/**
 * View for managing Budgets. Allows creating, editing budgets and entering budget lines per
 * account/period.
 */
...
public class BudgetsView extends VerticalLayout {
  ...
  private void configureGrid() {
    ...
    budgetGrid.addColumn(Budget::getName).setHeader("Name").setSortable(true).setFlexGrow(1);

    budgetGrid
        .addColumn(b -> b.getType().name())
        .setHeader("Type")
        .setSortable(true)
        .setAutoWidth(true);

    budgetGrid
        .addColumn(b -> b.getCurrency() != null ? b.getCurrency() : "NZD")
        .setHeader("Currency")
        .setAutoWidth(true);

    budgetGrid
        .addColumn(b -> b.isActive() ? "Active" : "Inactive")
        .setHeader("Status")
        .setAutoWidth(true);
  ...
  private void configureLineGrid() {
    ...
    lineGrid
        .addColumn(bl -> formatPeriod(bl.getPeriod()))
        .setHeader("Period")
        .setSortable(true)
        .setAutoWidth(true);

    lineGrid
        .addColumn(bl -> bl.getAccount().getCode())
        .setHeader("Account")
        .setSortable(true)
        .setAutoWidth(true);

    lineGrid.addColumn(bl -> bl.getAccount().getName()).setHeader("Account Name").setFlexGrow(1);

    lineGrid
        .addColumn(bl -> bl.getDepartment() != null ? bl.getDepartment().getCode() : "")
        .setHeader("Dept")
        .setAutoWidth(true);

    lineGrid
        .addColumn(bl -> formatCurrency(bl.getAmount()))
        .setHeader("Amount")
        .setSortable(true)
        .setAutoWidth(true);

    lineGrid
        .addComponentColumn(this::createLineActionButtons)
        .setHeader("Actions")
        .setAutoWidth(true)
        .setFlexGrow(0);
  }
  ...
  private void loadBudgetLines() {
    ...
    List<BudgetLine> lines =
        budgetService.findLinesByBudgetAndFiscalYear(selectedBudget, selectedFiscalYear.getId());
            ...
            if (isNew) {
              Budget newBudget =
                  budgetService.createBudget(
                      company, nameField.getValue().trim(), typeCombo.getValue(), null);
              ...
              Notification.show(
                      "Budget created successfully", 3000, Notification.Position.BOTTOM_START)
              ...
              Notification.show(
                      "Budget updated successfully", 3000, Notification.Position.BOTTOM_START)
                  .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            }
    ...
    saveButton.addClickListener(
        e -> {
          ...
          try {
            ...
            Notification.show(
                    "Budget line saved successfully", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            ...
            Notification.show(
                    "Error saving budget line: " + ex.getMessage(),
                    3000,
  ...
  private void openImportDialog() {
    ...
    Span instructions =
        new Span(
            "Upload a CSV file with budget data. Required columns: account_code, "
    ...
    updateExisting.setHelperText(
        "If unchecked, existing lines (same account/period/department) will be skipped");
        ...
        event -> {
          try {
            ...
            ImportResult preview =
                budgetImportService.previewImport(
    ...
    importButton.addClickListener(
        e -> {
          ...
          try {
            ...
            ImportResult result =
                budgetImportService.importBudgetLines(
                    new ByteArrayInputStream(uploadedBytes[0]),
                    selectedBudget,
                    user,
                    updateExisting.getValue());

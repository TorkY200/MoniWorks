...
/**
 * View for managing Payment Runs - batch supplier payments.
 * Allows creating payment runs, selecting bills to pay, and completing runs which generates
 * payment transactions and remittance advice PDFs.
 */
...
public class PaymentRunsView extends VerticalLayout {
    ...
    public PaymentRunsView(PaymentRunService paymentRunService,
    ...
    private void configureGrid() {
        ...
        runGrid.addColumn(run -> run.getBankAccount() != null ?
                run.getBankAccount().getName() : "")
        ...
        runGrid.addColumn(run -> formatMoney(paymentRunService.getRunTotal(run)))
            .setHeader("Total")
            .setSortable(true)
            .setAutoWidth(true);

        runGrid.addColumn(run -> run.getCreatedBy() != null ?
                run.getCreatedBy().getDisplayName() : "")
    ...
    private void updateDetailPanel() {
        ...
        details.add(createDetailRow("Bank Account:", selectedRun.getBankAccount() != null ?
                selectedRun.getBankAccount().getName() : ""));
        ...
        billsGrid.addColumn(prb -> prb.bill().getBillNumber())
            .setHeader("Bill #")
            .setAutoWidth(true);

        billsGrid.addColumn(prb -> prb.bill().getContact() != null ?
                prb.bill().getContact().getName() : "")
            .setHeader("Supplier")
            .setFlexGrow(1);

        billsGrid.addColumn(prb -> formatDate(prb.bill().getBillDate()))
            .setHeader("Bill Date")
            .setAutoWidth(true);

        billsGrid.addColumn(prb -> formatDate(prb.bill().getDueDate()))
            .setHeader("Due Date")
            .setAutoWidth(true);

        billsGrid.addColumn(prb -> formatMoney(prb.bill().getTotal()))
            .setHeader("Total")
            .setAutoWidth(true);

        billsGrid.addColumn(prb -> formatMoney(prb.amount()))
            .setHeader("Payment")
            .setAutoWidth(true);

        // Add remove button for draft runs
        if (selectedRun.isDraft()) {
            billsGrid.addComponentColumn(prb -> {
        ...
        Map<String, BigDecimal> bySupplier = runBills.stream()
            .collect(Collectors.groupingBy(
                prb -> prb.bill().getContact() != null ? prb.bill().getContact().getName() : "Unknown",
    ...
    /**
     * Opens the Create Payment Run wizard - a step-by-step dialog for creating a payment run.
     * Step 1: Select bank account and run date, filter criteria
     * Step 2: Review selected bills
     * Step 3: Confirm and optionally complete
     */
    ...
    private VerticalLayout createWizardStep1(Company company, Dialog dialog, List<SupplierBill> selectedBills) {
        VerticalLayout layout = new VerticalLayout();
        layout.setSizeFull();
        layout.setPadding(false);
        ...
        MultiSelectComboBox<Contact> supplierFilter = new MultiSelectComboBox<>("Specific Suppliers (Optional)");
        ...
        previewGrid.addColumn(SupplierBill::getBillNumber)
            .setHeader("Bill #")
            .setAutoWidth(true);

        previewGrid.addColumn(b -> b.getContact() != null ? b.getContact().getName() : "")
            .setHeader("Supplier")
            .setFlexGrow(1);

        previewGrid.addColumn(b -> formatDate(b.getBillDate()))
            .setHeader("Bill Date")
            .setAutoWidth(true);

        previewGrid.addColumn(b -> formatDate(b.getDueDate()))
            .setHeader("Due Date")
            .setAutoWidth(true);

        previewGrid.addColumn(b -> formatMoney(b.getBalance()))
            .setHeader("Balance")
            .setAutoWidth(true);
        ...
        loadBillsBtn.addClickListener(e -> {
            ...
            if (selectedSuppliers != null && !selectedSuppliers.isEmpty()) {
                Set<Long> supplierIds = selectedSuppliers.stream()
                    .map(Contact::getId)
                    .collect(Collectors.toSet());
                bills = bills.stream()
                    .filter(b -> b.getContact() != null && supplierIds.contains(b.getContact().getId()))
        ...
        previewGrid.asMultiSelect().addValueChangeListener(e -> {
        ...
        createDraftBtn.addClickListener(e -> {
            ...
            if (selectedBills.isEmpty()) {
                Notification.show("Please select at least one bill to pay", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }

            createPaymentRunDraft(bankAccountField.getValue(), runDateField.getValue(),
                    notesField.getValue(), selectedBills, dialog);
        });
        ...
        createAndCompleteBtn.addClickListener(e -> {
            if (bankAccountField.isEmpty()) {
                Notification.show("Please select a bank account", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            ...
            createAndCompletePaymentRun(bankAccountField.getValue(), runDateField.getValue(),
                    notesField.getValue(), selectedBills, dialog);
        });
    ...
    private void createPaymentRunDraft(Account bankAccount, LocalDate runDate, String notes,
                                        List<SupplierBill> bills, Dialog dialog) {
        try {
            Company company = companyContextService.getCurrentCompany();
            User currentUser = companyContextService.getCurrentUser();

            PaymentRun run = paymentRunService.createPaymentRun(company, runDate, bankAccount, currentUser);
            if (notes != null && !notes.isBlank()) {
                run.setNotes(notes);
            }
            paymentRunService.addBillsToRun(run, bills);

            Notification.show("Payment run created. Review and click 'Complete Run' to process.",
    ...
    private void createAndCompletePaymentRun(Account bankAccount, LocalDate runDate, String notes,
                                              List<SupplierBill> bills, Dialog dialog) {
        try {
            Company company = companyContextService.getCurrentCompany();
            User currentUser = companyContextService.getCurrentUser();
            ...
            Notification.show("Payment run completed. " + bills.size() + " bills paid. Remittance advice generated.",
                    5000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    ...
    private void openAddBillsDialog() {
        ...
        Set<Long> existingBillIds = paymentRunService.getRunBills(selectedRun).stream()
            .map(prb -> prb.bill().getId())
            .collect(Collectors.toSet());

        // Get available bills
        List<SupplierBill> availableBills = supplierBillService.findOutstandingByCompany(company).stream()
        ...
        billsGrid.addColumn(SupplierBill::getBillNumber)
            .setHeader("Bill #")
            .setAutoWidth(true);

        billsGrid.addColumn(b -> b.getContact() != null ? b.getContact().getName() : "")
            .setHeader("Supplier")
            .setFlexGrow(1);

        billsGrid.addColumn(b -> formatDate(b.getDueDate()))
            .setHeader("Due Date")
            .setAutoWidth(true);

        billsGrid.addColumn(b -> formatMoney(b.getBalance()))
            .setHeader("Balance")
            .setAutoWidth(true);
        ...
        billsGrid.asMultiSelect().addValueChangeListener(e -> {
        ...
        addBtn.addClickListener(e -> {
            ...
            try {
                ...
                Notification.show("Added " + selected.size() + " bills to payment run",
    ...
    private void completeRun() {
        ...
        try {
            ...
            Notification.show("Payment run completed. Remittance advice generated.",
                    5000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            ...
            Notification.show("Error completing run: " + ex.getMessage(),
    ...
    private void openEmailRemittanceDialog() {
        ...
        List<PaymentRunBill> runBills = paymentRunService.getRunBills(selectedRun);
        Set<Contact> suppliers = runBills.stream()
        ...
        selectAll.addValueChangeListener(e ->
            supplierCheckboxes.forEach(cb -> cb.setValue(e.getValue())));
        ...
        sendBtn.addClickListener(e -> {
            List<Contact> selectedSuppliers = supplierCheckboxes.stream()
                .filter(Checkbox::getValue)
                .map(checkboxToSupplier::get)
                .collect(Collectors.toList());

            if (selectedSuppliers.isEmpty()) {
                Notification.show("Please select at least one supplier", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
    ...
    private void sendRemittanceEmails(List<Contact> suppliers) {
        ...
        for (Contact supplier : suppliers) {
            try {
                ...
                EmailService.EmailResult result = emailService.sendRemittanceAdvice(
                    supplier, company, pdfContent, currentUser);
        ...
        if (failed == 0) {
            Notification.show("Remittance advice sent to " + sent + " supplier(s)",
                    5000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
        } else {
            Notification.show("Sent: " + sent + ", Failed: " + failed,
                    5000, Notification.Position.MIDDLE)

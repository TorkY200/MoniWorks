...
/**
 * View for managing Departments (cost centres).
 * Allows creating, editing, and deactivating departments.
 * Departments provide dimensional reporting on transactions.
 */
        ...
        if (searchTerm == null || searchTerm.isBlank()) {
            ...
            List<Department> filtered = departments.stream()
                .filter(d ->
                    ...
                    (d.getGroupName() != null && d.getGroupName().toLowerCase().contains(lowerSearch)) ||
                    (d.getClassification() != null && d.getClassification().toLowerCase().contains(lowerSearch)))
                .toList();
    ...
    private void openDepartmentDialog(Department department) {
        ...
        saveButton.addClickListener(e -> {
            ...
            if (!code.matches("^[A-Z0-9]+$")) {
                Notification.show("Code must contain only letters and numbers", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
            ...
            try {
                ...
                if (isNew) {
                    ...
                    Notification.show("Department created successfully", 3000, Notification.Position.BOTTOM_START)
                    ...
                    department.setClassification(classificationField.isEmpty() ? null : classificationField.getValue().trim());
                    department.setActive(activeCheckbox.getValue());
                    departmentService.save(department, null);

                    Notification.show("Department updated successfully", 3000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                }
    ...
    private void createDefaultDepartments() {
        try {
            ...
            if (!departmentService.findByCompany(company).isEmpty()) {
                Notification.show("Departments already exist for this company. Defaults not created.",

...
/**
 * Public view for accepting user invitations. Allows new users to create an account or existing
 * logged-in users to accept directly.
 */
...
public class AcceptInvitationView extends VerticalLayout implements BeforeEnterObserver {
  ...
  public void beforeEnter(BeforeEnterEvent event) {
    ...
    List<String> tokenParams =
        event.getLocation().getQueryParameters().getParameters().get("token");
    ...
    Paragraph info =
        new Paragraph(
            String.format(
                ...
                invitation.getCompany().getName(), invitation.getEmail()));
  ...
  private void showExistingUserFlow(User user) {
    ...
    if (!user.getEmail().equalsIgnoreCase(invitation.getEmail())) {
      ...
      Button logoutButton =
          new Button("Log Out", e -> UI.getCurrent().getPage().setLocation("/logout"));
  ...
  private void showNewUserFlow() {
    ...
    Paragraph info =
        new Paragraph("Complete the form below to create your account and join the company.");
  ...
  private void acceptAsNewUser(String displayName, String password) {
    InvitationResult result =
        invitationService.acceptInvitationNewUser(token, displayName, password);
  ...
  private void acceptAsExistingUser(User user) {
    ...
    if (result.success()) {
      Notification.show(
              "You have joined " + invitation.getCompany().getName() + "!",
              5000,
              Notification.Position.TOP_CENTER)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);

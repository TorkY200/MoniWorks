...
/**
 * View for managing roles and their permissions. Admin users can view system roles and create/edit
...
public class RolesView extends VerticalLayout {
  ...
  private void updateDetailLayout() {
    ...
    Map<String, List<Permission>> permsByCategory =
        selectedRole.getPermissions().stream()
            .collect(
                Collectors.groupingBy(
                    p -> p.getCategory() != null ? p.getCategory() : "Other", Collectors.toList()));
    ...
    permsByCategory.keySet().stream()
        ...
        .forEach(
            category -> {
              ...
              permsList.addClassNames(
                  LumoUtility.Margin.Left.MEDIUM, LumoUtility.Margin.Bottom.SMALL);
        ...
        new Button(
            ...
            e -> {
              ...
              try {
                ...
                roleService.createCompanyRole(
                    company, name.trim(), descField.getValue(), new HashSet<>());
                refreshGrid();
                dialog.close();
                Notification.show(
                        "Role created. Now add permissions to it.",
                        3000,
    ...
    permsByCategory.keySet().stream()
        ...
        .forEach(
            category -> {
              ...
              permsLayout.addClassNames(
                  LumoUtility.Margin.Left.MEDIUM, LumoUtility.Margin.Bottom.MEDIUM);
              ...
              selectAllBtn.addClickListener(
                  e -> categoryCheckboxes.forEach(cb -> cb.setValue(true)));
              selectNoneBtn.addClickListener(
                  e -> categoryCheckboxes.forEach(cb -> cb.setValue(false)));
  ...
  private void showViewPermissionsDialog(Role role) {
    ...
    Span notice =
        new Span(
            "System roles cannot be modified. Contact an administrator to create a custom role.");

...
/**
 * View for displaying the audit trail.
 * Shows all audit events for the current company with filtering and detail view.
...
public class AuditEventsView extends VerticalLayout {
    ...
    public AuditEventsView(AuditService auditService,
    ...
    private void configureGrid() {
        ...
        grid.addColumn(this::formatActor)
            .setHeader("Actor")
            .setSortable(true)
            .setAutoWidth(true);
        ...
        grid.addColumn(AuditEvent::getSummary)
            .setHeader("Summary")
            .setFlexGrow(1);

        grid.addComponentColumn(this::createViewButton)
            .setHeader("")
            .setAutoWidth(true)
            .setFlexGrow(0);
    }
    ...
    private HorizontalLayout createFilters() {
        ...
        Button clearBtn = new Button("Clear Filters", e -> {
    ...
    private void loadAuditEvents() {
        ...
        if (fromDateFilter.getValue() != null && toDateFilter.getValue() != null) {
            ...
            events = auditService.findByCompany(company, org.springframework.data.domain.Pageable.ofSize(500)).getContent();
        }
        ...
        if (eventTypeFilter.getValue() != null && !eventTypeFilter.getValue().isBlank()) {
            ...
            events = events.stream()
                .filter(e -> e.getEventType() != null && e.getEventType().toLowerCase().contains(filter))
                .toList();
        }
        ...
        if (entityTypeFilter.getValue() != null && !entityTypeFilter.getValue().isBlank()) {
            ...
            events = events.stream()
                .filter(e -> e.getEntityType() != null && e.getEntityType().toLowerCase().contains(filter))
                .toList();
        }
    ...
    private void openDetailDialog(AuditEvent event) {
        ...
        content.add(createDetailRow("Entity ID:", event.getEntityId() != null ? event.getEntityId().toString() : "-"));

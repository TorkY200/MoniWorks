...
/**
 * View for managing KPIs (Key Performance Indicators).
 * Allows creating, editing KPIs and entering values per period.
 */
...
public class KPIsView extends VerticalLayout {
    ...
    public KPIsView(KPIService kpiService,
    ...
    private void configureGrid() {
        ...
        kpiGrid.addColumn(KPI::getCode)
            .setHeader("Code")
            .setSortable(true)
            .setAutoWidth(true);

        kpiGrid.addColumn(KPI::getName)
            .setHeader("Name")
            .setSortable(true)
            .setFlexGrow(1);

        kpiGrid.addColumn(k -> k.getUnit() != null ? k.getUnit() : "")
            .setHeader("Unit")
            .setAutoWidth(true);

        kpiGrid.addColumn(k -> k.isActive() ? "Active" : "Inactive")
            .setHeader("Status")
            .setAutoWidth(true);

        kpiGrid.asSingleSelect().addValueChangeListener(e -> {
    ...
    private void updateDetailPanel() {
        ...
        if (selectedKPI.getUnit() != null && !selectedKPI.getUnit().isEmpty()) {
            header.getElement().appendChild(
                new Span(" (" + selectedKPI.getUnit() + ")").getElement());
        }
    ...
    private void configureValueGrid() {
        ...
        valueGrid.addColumn(kv -> formatPeriod(kv.getPeriod()))
            .setHeader("Period")
            .setSortable(true)
            .setAutoWidth(true);

        valueGrid.addColumn(kv -> formatValue(kv.getValue()))
            .setHeader("Value")
            .setSortable(true)
            .setAutoWidth(true);

        valueGrid.addColumn(kv -> kv.getNotes() != null ? kv.getNotes() : "")
            .setHeader("Notes")
            .setFlexGrow(1);

        valueGrid.addComponentColumn(this::createValueActionButtons)
            .setHeader("Actions")
            .setAutoWidth(true)
            .setFlexGrow(0);
    }
    ...
    private void loadKPIValues() {
        ...
        List<KPIValue> values = kpiService.getValuesForKPIAndFiscalYear(
            selectedKPI, selectedFiscalYear.getId());
    ...
    private void openKPIDialog(KPI kpi) {
        ...
        saveButton.addClickListener(e -> {
            ...
            try {
                ...
                if (isNew) {
                    KPI newKPI = kpiService.createKPI(
                        company,
                    ...
                    newKPI.setDescription(descriptionField.isEmpty() ? null : descriptionField.getValue().trim());
                    newKPI.setActive(activeCheckbox.getValue());
                    kpiService.save(newKPI, null);

                    Notification.show("KPI created successfully", 3000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                } else {
                    kpi.setName(nameField.getValue().trim());
                    kpi.setUnit(unitField.isEmpty() ? null : unitField.getValue().trim());
                    kpi.setDescription(descriptionField.isEmpty() ? null : descriptionField.getValue().trim());
                    kpi.setActive(activeCheckbox.getValue());
                    kpiService.save(kpi, null);

                    Notification.show("KPI updated successfully", 3000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                }
    ...
    private void openKPIValueDialog(KPIValue kpiValue) {
        ...
        saveButton.addClickListener(e -> {
            ...
            try {
                ...
                Notification.show("KPI value saved successfully", 3000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);

                dialog.close();
                loadKPIValues();
            } catch (Exception ex) {
                Notification.show("Error saving KPI value: " + ex.getMessage(), 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });
    ...
    private void createDefaultKPIs() {
        try {
            ...
            if (!kpiService.findByCompany(company).isEmpty()) {
                Notification.show("KPIs already exist for this company. Defaults not created.",

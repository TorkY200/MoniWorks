...
/**
 * View for managing Recurring Transaction Templates. Allows creating, editing, pausing, and
...
public class RecurringTemplatesView extends VerticalLayout {
  ...
  private void configureGrid() {
    ...
    templateGrid
        .addColumn(RecurringTemplate::getName)
        .setHeader("Name")
        .setSortable(true)
        .setFlexGrow(1);

    templateGrid
        .addColumn(t -> t.getTemplateType().name())
        .setHeader("Type")
        .setSortable(true)
        .setAutoWidth(true);

    templateGrid
        .addColumn(RecurringTemplate::getScheduleDescription)
        .setHeader("Schedule")
        .setSortable(false)
        .setAutoWidth(true);

    templateGrid
        .addColumn(t -> formatDate(t.getNextRunDate()))
        .setHeader("Next Run")
        .setSortable(true)
        .setAutoWidth(true);

    templateGrid.addComponentColumn(this::createStatusBadge).setHeader("Status").setAutoWidth(true);
  ...
  private void updateDetailPanel() {
    ...
    details.add(
        createDetailRow("Executions:", String.valueOf(selectedTemplate.getOccurrencesCount())));
    details.add(
        createDetailRow(
            "Mode:",
            selectedTemplate.getExecutionMode() == ExecutionMode.AUTO_POST
                ? "Auto-post"
                : "Create as draft"));
  ...
  private void configureLogGrid() {
    ...
    logGrid
        .addColumn(log -> formatDateTime(log.getRunAt()))
        .setHeader("Run At")
        .setSortable(true)
        .setAutoWidth(true);

    logGrid.addComponentColumn(this::createResultBadge).setHeader("Result").setAutoWidth(true);

    logGrid
        .addColumn(
            log ->
                log.getCreatedEntityId() != null
                    ? log.getCreatedEntityType() + " #" + log.getCreatedEntityId()
                    : "")
        .setHeader("Created Entity")
        .setAutoWidth(true);

    logGrid.addColumn(RecurrenceExecutionLog::getError).setHeader("Error").setFlexGrow(1);
  }
  ...
  private void cancelTemplate() {
    ...
    dialog.setText(
        "Are you sure you want to cancel this recurring template? This action cannot be undone.");
  ...
  private void deleteTemplate() {
    ...
    dialog.setText(
        "Are you sure you want to delete this recurring template? All execution history will also be deleted.");
    dialog.setCancelable(true);
  ...
  private void runTemplateNow() {
    ...
    if (result.isSuccess()) {
      Notification.show(
              "Template executed successfully. Created "
                  + result.getCreatedEntityType()
                  + " #"
                  + result.getCreatedEntityId(),
              5000,
              Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    } else {
      Notification.show(
              "Template execution failed: " + result.getError(), 5000, Notification.Position.MIDDLE)
  ...
  private void runDueTemplates() {
    ...
    List<RecurringTemplate> dueTemplates =
        templateService.findDueTemplates(company.getId(), LocalDate.now());
    ...
    String message =
        String.format(
            "Executed %d templates: %d successful, %d failed",
            dueTemplates.size(), successCount, failCount);
    Notification.show(message, 5000, Notification.Position.BOTTOM_START)
        .addThemeVariants(
            failCount > 0 ? NotificationVariant.LUMO_CONTRAST : NotificationVariant.LUMO_SUCCESS);
  ...
  private void openCreateDialog() {
    ...
    modeCombo.setItemLabelGenerator(
        m -> m == ExecutionMode.AUTO_POST ? "Auto-post" : "Create as draft");
    ...
    saveButton.addClickListener(
        e -> {
          if (nameField.isEmpty()
              || typeCombo.isEmpty()
              || frequencyCombo.isEmpty()
              || startDateField.isEmpty()
              || payloadArea.isEmpty()) {
            Notification.show(
          ...
          if ((type == TemplateType.INVOICE || type == TemplateType.BILL)
              && contactCombo.isEmpty()) {
            Notification.show(
                    "Contact is required for Invoice/Bill templates",
                    3000,
          ...
          try {
            ...
            template.setFrequencyInterval(
                intervalField.getValue() != null ? intervalField.getValue() : 1);
            ...
            Notification.show(
                    "Template created successfully", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    ...
    saveButton.addClickListener(
        e -> {
          ...
          try {
            ...
            Notification.show(
                    "Template updated successfully", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);

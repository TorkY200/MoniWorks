...
/**
 * View for managing Tax Codes (GST/VAT codes).
 * Allows creating, editing, and deactivating tax codes.
 * Tax codes define rates and types for tax calculations on transactions.
 */
...
public class TaxCodesView extends VerticalLayout {
    ...
    public TaxCodesView(TaxCodeService taxCodeService,
    ...
    private void configureGrid() {
        ...
        grid.addColumn(TaxCode::getName)
            .setHeader("Name")
            .setSortable(true)
            .setFlexGrow(1);
        ...
        grid.addColumn(TaxCode::getReportBox)
            .setHeader("Report Box")
            .setAutoWidth(true);
    ...
    private HorizontalLayout createActionButtons(TaxCode taxCode) {
        ...
        if (taxCode.isActive()) {
            ...
            deactivateBtn.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL, ButtonVariant.LUMO_ERROR);
            ...
            activateBtn.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL, ButtonVariant.LUMO_SUCCESS);
    ...
    private HorizontalLayout createToolbar() {
        ...
        HorizontalLayout actions = new HorizontalLayout(searchField, addButton, defaultsButton, refreshButton);
    ...
    private void filterTaxCodes(String searchTerm) {
        ...
        if (searchTerm == null || searchTerm.isBlank()) {
            ...
            List<TaxCode> filtered = taxCodes.stream()
                .filter(tc ->
    ...
    private void openTaxCodeDialog(TaxCode taxCode) {
        ...
        saveButton.addClickListener(e -> {
            ...
            if (rate.compareTo(BigDecimal.ZERO) < 0 || rate.compareTo(BigDecimal.ONE) > 0) {
                Notification.show("Rate must be between 0 and 1 (e.g., 0.15 for 15%)", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
            ...
            try {
                ...
                if (isNew) {
                    TaxCode newTaxCode = taxCodeService.createTaxCode(
                        company,
                    ...
                    newTaxCode.setReportBox(reportBoxField.getValue().isBlank() ? null : reportBoxField.getValue().trim());
                    newTaxCode.setActive(activeCheckbox.getValue());
                    taxCodeService.save(newTaxCode);

                    Notification.show("Tax code created successfully", 3000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                } else {
                    taxCode.setName(nameField.getValue().trim());
                    taxCode.setRate(rate);
                    taxCode.setType(typeCombo.getValue());
                    taxCode.setReportBox(reportBoxField.getValue().isBlank() ? null : reportBoxField.getValue().trim());
                    taxCode.setActive(activeCheckbox.getValue());
                    taxCodeService.save(taxCode);

                    Notification.show("Tax code updated successfully", 3000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                }
    ...
    private void createDefaultTaxCodes() {
        try {
            ...
            if (!taxCodeService.findByCompany(company).isEmpty()) {
                Notification.show("Tax codes already exist for this company. Defaults not created.",
            ...
            Notification.show("Default NZ GST tax codes created", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            loadTaxCodes();
        } catch (Exception ex) {
            Notification.show("Error creating defaults: " + ex.getMessage(),
                3000, Notification.Position.MIDDLE)
                .addThemeVariants(NotificationVariant.LUMO_ERROR);
        }
    }

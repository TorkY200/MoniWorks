...
/**
 * View for managing users and company memberships. Only accessible to users with MANAGE_USERS
 * permission.
 */
...
public class UsersView extends VerticalLayout {
  ...
  private HorizontalLayout createToolbar() {
    ...
    HorizontalLayout toolbar =
        new HorizontalLayout(title, addUserBtn, inviteByEmailBtn, inviteExistingBtn);
  ...
  private void updateMembershipDetail() {
    ...
    UserSecurityLevel securityLevel =
        securityLevelRepository.findByUserAndCompany(user, company).orElse(null);
    ...
    Span securityInfo =
        new Span(
            "Current level: "
                + currentLevel
                + " (can view accounts with security level ≤ "
                + currentLevel
                + ")");
    ...
    setSecurityBtn.addClickListener(
        e -> showSecurityLevelDialog(selectedMembership, securityLevel));
    ...
    removeBtn.setEnabled(
        selectedMembership.getStatus() == CompanyMembership.MembershipStatus.ACTIVE);
  ...
  private void updateInvitationDetail() {
    ...
    Span nameLabel =
        new Span(
            "Name: "
                + (selectedInvitation.getDisplayName() != null
                    ? selectedInvitation.getDisplayName()
                    : "-"));
    ...
    Span invitedByLabel =
        new Span(
            "Invited by: "
                + (selectedInvitation.getInvitedBy() != null
                    ? selectedInvitation.getInvitedBy().getDisplayName()
                    : "-"));
    Span createdLabel = new Span("Created: " + formatter.format(selectedInvitation.getCreatedAt()));

    // Status-specific info
    if (selectedInvitation.isExpired()
        && selectedInvitation.getStatus() == UserInvitation.InvitationStatus.PENDING) {
    ...
    if (selectedInvitation.getStatus() == UserInvitation.InvitationStatus.ACCEPTED) {
      Span acceptedAtLabel =
          new Span("Accepted: " + formatter.format(selectedInvitation.getAcceptedAt()));
      Span acceptedUserLabel =
          new Span("Accepted by user: " + selectedInvitation.getAcceptedUser().getEmail());
    ...
    if (selectedInvitation.getStatus() == UserInvitation.InvitationStatus.PENDING) {
      ...
      copyLinkBtn.addClickListener(
          e -> {
            ...
            Notification.show(
                    "Invitation link copied to clipboard", 3000, Notification.Position.BOTTOM_START)
  ...
  private void showAddUserDialog() {
    ...
    roleCombo.setItems(
        roleService.findAvailableRolesForCompany(companyContextService.getCurrentCompany()));
              ...
              try {
                User user =
                    userService.createUser(
                        emailField.getValue(), nameField.getValue(), passwordField.getValue());

                userService.addToCompany(
                    user, companyContextService.getCurrentCompany(), roleCombo.getValue());

                Notification.show(
                        "User created successfully", 3000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                dialog.close();
                refreshGrid();
              } catch (Exception ex) {
  ...
  private void showInviteByEmailDialog() {
    ...
    Span infoText =
        new Span(
            "The recipient will receive an email with a link to create their account "
        ...
        new Button(
            ...
            e -> {
              ...
              try {
                ...
                Company company = companyContextService.getCurrentCompany();
                String displayName =
                    nameField.getValue().isBlank() ? null : nameField.getValue().trim();
                ...
                if (result.success()) {
                  Notification.show(
                          "Invitation sent to " + emailField.getValue(),
                          3000,
                        ...
                        .orElseThrow(
                            () ->
                                new IllegalArgumentException(
                                    "User not found: " + emailField.getValue()));
  ...
  private void resendInvitation(UserInvitation invitation) {
    ...
    if (result.success()) {
      Notification.show(
              "Invitation resent to " + invitation.getEmail(),
              3000,
              Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    } else {
  ...
  private void removeMembership(CompanyMembership membership) {
    ...
    if (currentUser != null && currentUser.getId().equals(membership.getUser().getId())) {
      Notification.show(
              "You cannot remove yourself from the company", 3000, Notification.Position.MIDDLE)
          .addThemeVariants(NotificationVariant.LUMO_ERROR);
      return;
    }
  ...
  private void showSecurityLevelDialog(
      CompanyMembership membership, UserSecurityLevel existingLevel) {
    Dialog dialog = new Dialog();
    ...
    Span description =
        new Span(
            "Security levels control which accounts a user can view. "
    ...
    Span examples =
        new Span(
            "Examples: Level 0 = basic accounts, Level 1+ = restricted accounts (e.g., payroll)");
    ...
    levelField.setHelperText(
        "User can view accounts with security level ≤ this value (0 = basic only)");
        ...
        new Button(
            ...
            e -> {
              if (levelField.getValue() == null) {
                Notification.show(
                        "Please enter a security level", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
              }
              ...
              try {
                ...
                Notification.show(
                        "Security level updated to " + newLevel,
                        3000,
                        Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                dialog.close();

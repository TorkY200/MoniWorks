 ...
 * Displays search results across all entity types with navigation.
 */
...
public class GlobalSearchView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public GlobalSearchView(GlobalSearchService globalSearchService, CompanyContextService companyContextService) {
        ...
        searchField.setPlaceholder("Search transactions, contacts, invoices, bills, products, accounts...");
    ...
    private Grid<GlobalSearchResult> createResultsGrid() {
        ...
        grid.addColumn(new ComponentRenderer<>(result -> {
            ...
            typeLabel.addClassNames(LumoUtility.FontSize.SMALL, LumoUtility.TextColor.SECONDARY);
        ...
        grid.addColumn(new ComponentRenderer<>(result -> {
            ...
            return layout;
        })).setHeader("Details").setFlexGrow(2);
        ...
        grid.addColumn(new ComponentRenderer<>(result -> {
            ...
            switch (result.status().toUpperCase()) {
                case "POSTED", "ISSUED", "PAID", "ACTIVE" -> badge.getElement().getThemeList().add("success");
    ...
    private Div createHelpSection() {
        Div help = new Div();
        help.addClassNames(LumoUtility.Padding.LARGE, LumoUtility.Background.CONTRAST_5, LumoUtility.BorderRadius.MEDIUM);
        ...
        tips.add(createTip("type:invoice", "Search only invoices (also: bill, transaction, contact, product, account)"));
        tips.add(createTip("status:overdue", "Filter by status (draft, posted, issued, overdue, paid, unpaid)"));
    ...
    private void performSearch() {
        ...
        UI.getCurrent().getPage().getHistory().pushState(null,
            "search?q=" + java.net.URLEncoder.encode(query, java.nio.charset.StandardCharsets.UTF_8));
    }

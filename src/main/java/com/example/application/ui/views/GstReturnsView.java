 ...
 *
 * <p>Features: - Generate new GST returns for a period - View return summary and box details -
 * Drilldown to transactions for each box - Finalize and mark returns as filed
 */
...
public class GstReturnsView extends VerticalLayout {
  ...
  private void configureGrid() {
    ...
    returnsGrid
        .addColumn(
            r -> r.getStartDate().format(DATE_FORMAT) + " - " + r.getEndDate().format(DATE_FORMAT))
        .setHeader("Period")
        .setSortable(true)
        .setAutoWidth(true);

    returnsGrid.addColumn(r -> r.getBasis().name()).setHeader("Basis").setAutoWidth(true);

    returnsGrid
        .addColumn(r -> formatCurrency(r.getTotalSales()))
        .setHeader("Total Sales")
        .setAutoWidth(true);

    returnsGrid
        .addColumn(r -> formatCurrency(r.getOutputTax()))
        .setHeader("Output Tax")
        .setAutoWidth(true);

    returnsGrid
        .addColumn(r -> formatCurrency(r.getInputTax()))
        .setHeader("Input Tax")
        .setAutoWidth(true);

    returnsGrid
        .addColumn(r -> formatCurrency(r.getTaxPayable()))
        .setHeader("Tax Payable")
        .setAutoWidth(true);

    returnsGrid
        .addColumn(r -> r.getStatus().name())
        .setHeader("Status")
        .setSortable(true)
        .setAutoWidth(true);

    returnsGrid
        .addComponentColumn(this::createActionButtons)
        .setHeader("Actions")
        .setAutoWidth(true)
        .setFlexGrow(0);
  }
  ...
  private void openGenerateDialog() {
    ...
    startDatePicker.addValueChangeListener(
        e -> {
          if (e.getValue() != null) {
            endDatePicker.setValue(
                e.getValue().plusMonths(2).withDayOfMonth(1).plusMonths(1).minusDays(1));
          }
        });
    ...
    generateBtn.addClickListener(
        e -> {
          if (startDatePicker.isEmpty() || endDatePicker.isEmpty()) {
            Notification.show(
                    "Please select start and end dates", 3000, Notification.Position.MIDDLE)
          ...
          try {
            TaxReturn taxReturn =
                taxReturnService.generateReturn(
                    company,
            ...
            Notification.show(
                    "GST return generated successfully", 3000, Notification.Position.BOTTOM_START)
  ...
  private void openViewDialog(TaxReturn taxReturn) {
    ...
    dialog.setHeaderTitle(
        "GST Return: "
            + taxReturn.getStartDate().format(DATE_FORMAT)
            + " to "
            + taxReturn.getEndDate().format(DATE_FORMAT));
    ...
    summaryGrid.addColumn(DrilldownSummaryRow::label).setHeader("Box").setFlexGrow(1);

    summaryGrid.addColumn(r -> formatCurrency(r.amount())).setHeader("Amount").setAutoWidth(true);

    // Populate summary from return lines with drilldown support
    List<DrilldownSummaryRow> summaryRows =
        taxReturn.getLines().stream()
            .map(
                line ->
                    new DrilldownSummaryRow(
    ...
    if (taxReturn.getGeneratedAt() != null) {
      statusInfo.add(
          new Paragraph("Generated: " + taxReturn.getGeneratedAt().toString().substring(0, 16)));
    }
    if (taxReturn.getFinalizedAt() != null) {
      statusInfo.add(
          new Paragraph("Finalized: " + taxReturn.getFinalizedAt().toString().substring(0, 16)));
    }
  ...
  private void finalizeReturn(TaxReturn taxReturn) {
    try {
      ...
      Notification.show("Return finalized", 3000, Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
      loadReturns();
  ...
  private void deleteReturn(TaxReturn taxReturn) {
    try {
      ...
      Notification.show("Return deleted", 3000, Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
      loadReturns();
    } catch (IllegalStateException ex) {
      Notification.show(ex.getMessage(), 3000, Notification.Position.MIDDLE)
  ...
  private void markAsFiled(TaxReturn taxReturn) {
    try {
      ...
      Notification.show("Return marked as filed", 3000, Notification.Position.BOTTOM_START)
          .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
      loadReturns();
    } catch (IllegalStateException ex) {
      Notification.show(ex.getMessage(), 3000, Notification.Position.MIDDLE)
  ...
  private record DrilldownSummaryRow(
      String label, BigDecimal amount, String boxCode, LocalDate startDate, LocalDate endDate) {}
  ...
  private void openTaxLineDrilldownDialog(String boxCode, LocalDate startDate, LocalDate endDate) {
    ...
    // Period info
    Span periodInfo =
        new Span("Period: " + startDate.format(DATE_FORMAT) + " to " + endDate.format(DATE_FORMAT));
    periodInfo.getStyle().set("color", "var(--lumo-secondary-text-color)");

    // Fetch tax lines for this box
    Company company = companyContextService.getCurrentCompany();
    List<TaxLine> taxLines =
        taxLineRepository.findByCompanyAndReportBoxAndDateRange(
            company, boxCode, startDate, endDate);
    ...
    grid.addColumn(TaxLine::getTaxCode).setHeader("Tax Code").setAutoWidth(true).setFlexGrow(0);
    ...
    BigDecimal totalTaxable =
        taxLines.stream().map(TaxLine::getTaxableAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal totalTax =
        taxLines.stream().map(TaxLine::getTaxAmount).reduce(BigDecimal.ZERO, BigDecimal::add);

...
/**
 * View for managing company settings including PDF template customization. Allows admins to
 * configure company branding, logo, contact details, and PDF generation settings.
 */
...
public class CompanySettingsView extends VerticalLayout {
  ...
  private VerticalLayout createPdfSettingsTab() {
    ...
    Span logoHelp =
        new Span(
            "Upload a logo image (PNG, JPEG, GIF) to appear on invoices, statements, and reports.");
    logoHelp.addClassNames(LumoUtility.TextColor.SECONDARY, LumoUtility.FontSize.SMALL);
    layout.add(logoHelp);

    // Logo preview
    logoPreview = new Div();
    logoPreview.addClassNames(
    ...
    upload.addSucceededListener(
        event -> {
          try {
            ...
            updateLogoPreview();
            Notification.show(
                    "Logo uploaded successfully", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
          } catch (IOException e) {
            Notification.show(
                    "Failed to upload logo: " + e.getMessage(), 5000, Notification.Position.MIDDLE)
    ...
    footerTextField.setValue(
        pdfSettings.getFooterText() != null ? pdfSettings.getFooterText() : "");
    ...
    Span bankHelp =
        new Span("These details appear in the payment information section of invoices.");
    ...
    bankAccountNameField.setValue(
        pdfSettings.getBankAccountName() != null ? pdfSettings.getBankAccountName() : "");
    ...
    bankAccountNumberField.setValue(
        pdfSettings.getBankAccountNumber() != null ? pdfSettings.getBankAccountNumber() : "");
  ...
  private VerticalLayout createCompanyDetailsTab() {
    ...
    companyAddressField.setValue(
        pdfSettings.getCompanyAddress() != null ? pdfSettings.getCompanyAddress() : "");
    ...
    companyPhoneField.setValue(
        pdfSettings.getCompanyPhone() != null ? pdfSettings.getCompanyPhone() : "");
    companyPhoneField.setPlaceholder("+64 9 123 4567");

    companyEmailField = new TextField("Email");
    companyEmailField.setValue(
        pdfSettings.getCompanyEmail() != null ? pdfSettings.getCompanyEmail() : "");
    companyEmailField.setPlaceholder("accounts@example.com");

    companyWebsiteField = new TextField("Website");
    companyWebsiteField.setValue(
        pdfSettings.getCompanyWebsite() != null ? pdfSettings.getCompanyWebsite() : "");
    ...
    detailsForm.add(
        companyAddressField, companyPhoneField, companyEmailField, companyWebsiteField, taxIdField);
  ...
  private VerticalLayout createTaxSettingsTab() {
    ...
    taxBasisCombo.setItemLabelGenerator(
        basis -> {
          switch (basis) {
            case CASH:
              return "Cash Basis";
            case INVOICE:
              return "Invoice/Accrual Basis";
            default:
              return basis.name();
          }
        });
    ...
    Span cashExplanation =
        new Span(
            "Cash Basis: Tax is reported when payments are received or made. "
    ...
    Span invoiceExplanation =
        new Span(
            "Invoice Basis: Tax is reported when invoices or bills are issued. "
    ...
    Span implementationNote =
        new Span(
            "Note: The selected tax basis will be used as the default when generating GST returns. "
      ...
      try {
        ...
        if (logoBytes != null && logoBytes.length > 0) {
          StreamResource resource =
              new StreamResource("logo", () -> new ByteArrayInputStream(logoBytes));
  ...
  private void saveAllSettings() {
    ...
    try {
      ...
      Notification.show(
              "Failed to save settings: " + e.getMessage(), 5000, Notification.Position.MIDDLE)
          .addThemeVariants(NotificationVariant.LUMO_ERROR);
    }
  }
}

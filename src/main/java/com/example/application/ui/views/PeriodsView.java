...
/**
 * View for managing Fiscal Years and Periods. Allows creating fiscal years and locking/unlocking
 * periods.
 */
...
public class PeriodsView extends VerticalLayout {
  ...
  public PeriodsView(
      FiscalYearService fiscalYearService, CompanyContextService companyContextService) {
  ...
  private VerticalLayout createContent() {
    ...
    fiscalYearCombo.setItemLabelGenerator(
        fy ->
            fy.getLabel()
                + " ("
                + fy.getStartDate().format(DATE_FORMAT)
                + " - "
  ...
  private void configurePeriodsGrid() {
    ...
    periodsGrid
        .addColumn(p -> "Period " + p.getPeriodIndex())
        .setHeader("Period")
        .setAutoWidth(true)
        .setFlexGrow(0);

    periodsGrid
        .addColumn(p -> p.getStartDate().format(DATE_FORMAT))
        .setHeader("Start Date")
        .setAutoWidth(true);

    periodsGrid
        .addColumn(p -> p.getEndDate().format(DATE_FORMAT))
        .setHeader("End Date")
        .setAutoWidth(true);

    periodsGrid.addColumn(p -> p.getStatus().name()).setHeader("Status").setAutoWidth(true);

    periodsGrid
        .addComponentColumn(this::createPeriodActions)
        .setHeader("Actions")
        .setAutoWidth(true)
        .setFlexGrow(0);
  }
        ...
        e -> {
          ...
          try {
            fiscalYearService.createFiscalYear(
                company, startDatePicker.getValue(), labelField.getValue().trim());

            Notification.show(
                    "Fiscal year created successfully", 3000, Notification.Position.BOTTOM_START)
                .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            ...
            Notification.show(
                    "Error creating fiscal year: " + ex.getMessage(),
                    3000,
  ...
  private void addNextFiscalYear() {
    ...
    try {
      ...
      Notification.show(
              "Fiscal year " + newYear.getLabel() + " created",
              3000,

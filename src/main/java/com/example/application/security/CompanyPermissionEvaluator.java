 ...
 *
 * <p>Supports two evaluation modes: 1. Simple permission check: hasPermission(authentication, null,
 * 'PERMISSION_NAME') - Checks if user has the permission in ANY of their companies 2.
 * Company-scoped check: hasPermission(authentication, companyId, 'PERMISSION_NAME') - Checks if
 * user has the permission in the SPECIFIC company
 */
@Component
public class CompanyPermissionEvaluator implements PermissionEvaluator {
  ...
  public CompanyPermissionEvaluator(
      UserRepository userRepository, CompanyMembershipRepository membershipRepository) {
  ...
  /**
   * Checks if the user has the given permission in ANY of their companies. This is used for
   * simple @PreAuthorize checks without company context.
   */
  ...
  /**
   * Checks if the user has the given permission in a SPECIFIC company. This is used for
   * company-scoped @PreAuthorize checks.
   */
  private boolean hasCompanyPermission(
      Authentication authentication, Long companyId, String permissionName) {
    ...
    Optional<CompanyMembership> membershipOpt =
        membershipRepository.findByUserIdAndCompanyId(user.getId(), companyId);

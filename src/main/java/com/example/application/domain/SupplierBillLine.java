...
/**
 * Represents a single line item on a supplier bill. Lines can reference a Product for auto-fill or
 * use free-form descriptions. Tax is calculated based on tax code and supports tax-inclusive
 * pricing.
 */
@Entity
...
public class SupplierBillLine {
  ...
  /**
   * Calculates line total and tax amount. Assumes tax-inclusive pricing (NZ standard) - extracts
   * tax from total.
   */
  public void calculateTotals() {
    BigDecimal grossTotal = quantity.multiply(unitPrice).setScale(2, RoundingMode.HALF_UP);

    if (taxRate != null && taxRate.compareTo(BigDecimal.ZERO) > 0) {
      // Tax-inclusive: extract tax from gross amount
      // Tax = Gross - (Gross / (1 + rate))
      BigDecimal divisor =
          BigDecimal.ONE.add(taxRate.divide(BigDecimal.valueOf(100), 4, RoundingMode.HALF_UP));

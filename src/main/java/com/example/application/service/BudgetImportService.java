 ...
 *
 * <p>CSV format expects columns: - account_code (required) - Account code to budget - period_date
 * (required) - Date within the period (YYYY-MM-DD) or period start date - amount (required) -
 * Budget amount - department_code (optional) - Department code for departmental budgeting
 *
 * <p>The import targets a specific budget and updates or creates budget lines as needed.
 */
@Service
@Transactional
public class BudgetImportService {
   ...
   */
  public ImportResult importBudgetLines(
      InputStream csvStream, Budget budget, User importedBy, boolean updateExisting)
      throws IOException {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        try {
          ...
          ProcessRowResult result =
              processRow(
                  values, columnMap, budget, company, updateExisting, previewOnly, lineNumber);
    ...
    if (!previewOnly && (imported > 0 || updated > 0)) {
      auditService.logEvent(
          ...
          "Imported "
              ...
              + ", skipped "
              + skipped
              + " budget lines from CSV into budget '"
              + budget.getName()
              + "'");
    }
  ...
  private ProcessRowResult processRow(
    ...
    if (account == null) {
      return new ProcessRowResult(
          RowAction.ERROR, "Line " + lineNumber + ": Account '" + accountCode + "' not found");
    }
    ...
    try {
      ...
      return new ProcessRowResult(
          RowAction.ERROR, "Line " + lineNumber + ": Invalid amount '" + amountStr + "'");
    }
    ...
    if (deptCode != null && !deptCode.isBlank()) {
      ...
      if (department == null) {
        return new ProcessRowResult(
            RowAction.ERROR, "Line " + lineNumber + ": Department '" + deptCode + "' not found");
      }
    }
    ...
    if (existing.isPresent()) {
      if (!updateExisting) {
        return new ProcessRowResult(
            ...
            "Line "
                + lineNumber
                + ": Budget line already exists for account "
                + accountCode
                + " in period "
                + period.getStartDate());
      }

...
/**
 * Service for managing KPIs and KPI values. KPIs track off-ledger metrics for dashboard and
 * reporting purposes.
 */
@Service
@Transactional
public class KPIService {
  ...
  public KPI createKPI(Company company, String code, String name, String unit, User actor) {
    ...
    auditService.logEvent(
        company, actor, "KPI_CREATE", "KPI", kpi.getId(), "Created KPI: " + code + " - " + name);
    ...
    if (!isNew) {
      auditService.logEvent(
          ...
          Map.of(
              ...
              saved.getName(),
              "unit",
              saved.getUnit() != null ? saved.getUnit() : "",
              "active",
              saved.isActive()));
    }
  ...
  /**
   * Creates default KPIs for a new company. Called during company setup to provide sensible
   * defaults.
   */

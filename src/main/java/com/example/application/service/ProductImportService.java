 ...
 * - code (required, max 31 chars)
 * - name (required, max 100 chars)
 *
 * Optional fields: description, category, buyPrice, sellPrice, taxCode,
 * barcode, stickyNote, salesAccountCode, purchaseAccountCode
 */
@Service
@Transactional
public class ProductImportService {
     ...
     */
    public ImportResult importProducts(InputStream csvStream, Company company, User importedBy,
    ...
    private ImportResult processImport(InputStream csvStream, Company company, User importedBy,
        ...
        if (!previewOnly && (imported > 0 || updated > 0)) {
            auditService.logEvent(company, importedBy, "PRODUCTS_IMPORTED", "Product", null,
                "Imported " + imported + " new, updated " + updated + ", skipped " + skipped + " products from CSV");
        }
    ...
    private void populateProduct(Product product, String[] values, Map<String, Integer> columnMap, Company company) {
        // Name
        String name = getColumnValue(values, columnMap, "name");
        if (name != null && !name.isBlank()) {
        ...
        if (salesAccountCode != null && !salesAccountCode.isBlank()) {
            accountRepository.findByCompanyAndCode(company, salesAccountCode.trim())
        ...
        if (purchaseAccountCode != null && !purchaseAccountCode.isBlank()) {
            accountRepository.findByCompanyAndCode(company, purchaseAccountCode.trim())

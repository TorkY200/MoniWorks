 ...
 * Handles file upload, storage, retrieval, and linking to entities.
 * Files are stored outside the database with SHA-256 checksums for integrity.
 */
@Service
@Transactional
public class AttachmentService {
     ...
     * Supports deduplication by checksum - if the same file already exists,
     ...
     */
    public Attachment uploadFile(Company company, String filename, String mimeType,
                                  byte[] content, User uploadedBy) {
        // Validate MIME type
        if (!ALLOWED_MIME_TYPES.contains(mimeType)) {
            throw new IllegalArgumentException(
                "File type not allowed: " + mimeType + ". Allowed types: PDF, JPEG, PNG, GIF, WEBP, TIFF, BMP");
        }
        ...
        Optional<Attachment> existing = attachmentRepository.findByCompanyAndChecksumSha256(company, checksum);
        if (existing.isPresent()) {
            log.info("File with same checksum already exists, returning existing attachment: {}",
                existing.get().getId());
            return existing.get();
        }
        ...
        Attachment attachment = new Attachment(
            company, filename, mimeType, (long) content.length,
            checksum, storageKey, uploadedBy);
        attachment = attachmentRepository.save(attachment);

        // Log audit event
        auditService.logEvent(company, uploadedBy, "ATTACHMENT_UPLOADED",
    ...
    public AttachmentLink linkToEntity(Attachment attachment, EntityType entityType, Long entityId) {
        ...
        Optional<AttachmentLink> existing = linkRepository.findByAttachmentAndEntityTypeAndEntityId(
     ...
     */
    public Attachment uploadAndLink(Company company, String filename, String mimeType,
                                     byte[] content, User uploadedBy,
    ...
    public byte[] getFileContent(Attachment attachment) {
        ...
        try {
            ...
            if (!checksum.equals(attachment.getChecksumSha256())) {
                log.error("Checksum mismatch for attachment {}: expected {}, got {}",
                    attachment.getId(), attachment.getChecksumSha256(), checksum);
     ...
     * Also removes all links to the attachment.
     */
    public void deleteAttachment(Attachment attachment, User deletedBy) {
        ...
        auditService.logEvent(attachment.getCompany(), deletedBy, "ATTACHMENT_DELETED",
            "ATTACHMENT", attachment.getId(),
    ...
    private String generateStorageKey(Company company, String filename) {
        ...
        return String.format("%d/%d/%02d/%s_%s",
            company.getId(), now.getYear(), now.getMonthValue(), uuid, sanitized);
    }

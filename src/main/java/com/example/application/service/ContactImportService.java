 ...
 *
 * <p>Supports flexible column mapping with required fields: - code (required, max 11 chars) - name
 * (required, max 100 chars) - type (optional, default CUSTOMER)
 *
 * <p>Optional fields: category, email, phone, mobile, addressLine1, addressLine2, city, region,
 * postalCode, country, website, paymentTerms, creditLimit, bankName, bankAccountNumber,
...
public class ContactImportService {
  ...
  private void populateContact(Contact contact, String[] values, Map<String, Integer> columnMap) {
    ...
    setIfPresent(
        contact::setBankAccountNumber, getColumnValue(values, columnMap, "bankaccountnumber"), 50);
    setIfPresent(contact::setBankRouting, getColumnValue(values, columnMap, "bankrouting"), 50);

    // Tax override
    setIfPresent(
        contact::setTaxOverrideCode, getColumnValue(values, columnMap, "taxoverridecode"), 10);
  }

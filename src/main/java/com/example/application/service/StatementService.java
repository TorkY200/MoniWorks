...
/**
 * Service for generating customer statements. Supports both open-item statements (showing
 * outstanding invoices with aging) and balance-forward statements (showing activity over a period).
 */
@Service
...
public class StatementService {
  ...
  private static final Font TITLE_FONT =
      new Font(Font.HELVETICA, 20, Font.BOLD, new Color(52, 73, 94));
  ...
  /**
   * DTO for balance-forward statement data (spec 09). Shows activity over a period with
  ...
  public record BalanceForwardStatementData(
    ...
    /**
     * A line in a balance-forward statement. Can represent an invoice (debit) or payment/credit
     * note (credit).
     */
  ...
  public StatementData generateStatementData(Company company, Contact contact, LocalDate asOfDate) {
    ...
    List<SalesInvoice> unpaidInvoices =
        invoiceRepository.findOutstandingByCompanyAndContact(company, contact).stream()
    ...
    for (SalesInvoice invoice : unpaidInvoices) {
      ...
      lines.add(
          new StatementData.StatementLine(
              invoice,
    ...
    BigDecimal totalBalance =
        currentBalance.add(days30Balance).add(days60Balance).add(days90PlusBalance);
  ...
  public byte[] generateStatementPdf(StatementData data) {
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      ...
      log.info(
          "Generated statement PDF for customer {}: {} bytes",
          data.contact().getCode(),
          baos.size());
  ...
  /**
   * Generates balance-forward statement data for a customer (spec 09). Shows activity over a period
   * with opening/closing balances and running total.
   *
   * @param company The company
   * @param contact The customer contact
  ...
  public BalanceForwardStatementData generateBalanceForwardStatementData(
    ...
    BigDecimal openingBalance =
        invoiceRepository.sumOutstandingByContactAsOfDate(company, contact, periodStart);
    if (openingBalance == null) {
      openingBalance = BigDecimal.ZERO;
    }

    // Get all invoices/credit notes issued in the period
    List<SalesInvoice> periodInvoices =
        invoiceRepository.findByCompanyAndContactAndDateRange(
    ...
    List<ReceivableAllocation> periodAllocations =
        allocationRepository.findByContactAndDateRange(company, contact, startInstant, endInstant);
    ...
    for (ReceivableAllocation allocation : periodAllocations) {
      LocalDate allocationDate =
          allocation.getAllocatedAt().atZone(ZoneId.systemDefault()).toLocalDate();
      activities.add(
          new ActivityItem(
              allocationDate,
    ...
    // Sort by date
    activities.sort(
        Comparator.comparing(ActivityItem::date)
    ...
    for (ActivityItem activity : activities) {
      ...
      lines.add(
          new BalanceForwardStatementData.BalanceForwardLine(
              activity.date,
              activity.reference,
  ...
  private record ActivityItem(
      LocalDate date, String reference, String description, BigDecimal debit, BigDecimal credit) {}
   ...
   * @return byte array containing the PDF
   */
  public byte[] generateBalanceForwardStatementPdf(
      Company company, Contact contact, LocalDate periodStart, LocalDate periodEnd) {
    BalanceForwardStatementData data =
        generateBalanceForwardStatementData(company, contact, periodStart, periodEnd);
  ...
  public byte[] generateBalanceForwardStatementPdf(BalanceForwardStatementData data) {
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      ...
      log.info(
          "Generated balance-forward statement PDF for customer {}: {} bytes",
          data.contact().getCode(),
          baos.size());
      ...
      log.error(
          "Failed to generate balance-forward statement PDF for customer {}",
          data.contact().getCode(),
          e);
      throw new RuntimeException(
          "Failed to generate balance-forward statement PDF: " + e.getMessage(), e);
    }
  }
  ...
  private void addBalanceForwardHeader(Document document, BalanceForwardStatementData data)
    ...
    Paragraph dateRange =
        new Paragraph(
            data.periodStart().format(shortDateFormatter)
                + " to "
                + data.periodEnd().format(shortDateFormatter),
            NORMAL_FONT);
  ...
  private void addOpeningBalance(Document document, BalanceForwardStatementData data)
    ...
    PdfPCell labelCell =
        new PdfPCell(
            new Phrase(
                "Opening Balance as at " + data.periodStart().format(shortDateFormatter),
                BOLD_FONT));
    ...
    PdfPCell amountCell =
        new PdfPCell(new Phrase(formatCurrency(data.openingBalance()), BOLD_FONT));
  ...
  private void addBalanceForwardTransactionList(Document document, BalanceForwardStatementData data)
    ...
    PdfPCell totalDebitCell =
        new PdfPCell(new Phrase(formatCurrency(data.totalDebits()), BOLD_FONT));
    ...
    PdfPCell totalCreditCell =
        new PdfPCell(new Phrase(formatCurrency(data.totalCredits()), BOLD_FONT));
  ...
  private void addBalanceForwardFooter(Document document, BalanceForwardStatementData data)
    ...
    PdfPCell labelCell =
        new PdfPCell(
            new Phrase(
                "Closing Balance as at " + data.periodEnd().format(shortDateFormatter), BOLD_FONT));
    ...
    Font balanceFont =
        data.closingBalance().compareTo(BigDecimal.ZERO) > 0
            ? new Font(Font.HELVETICA, 12, Font.BOLD, ACCENT_COLOR)
            : new Font(Font.HELVETICA, 12, Font.BOLD, SUCCESS_COLOR);
    PdfPCell amountCell =
        new PdfPCell(new Phrase(formatCurrency(data.closingBalance()), balanceFont));
  ...
  private void addHeader(Document document, StatementData data) throws DocumentException {
    ...
    Paragraph dateInfo =
        new Paragraph("As at " + data.statementDate().format(dateFormatter), NORMAL_FONT);
  ...
  private void addAgingSummary(Document document, StatementData data) throws DocumentException {
    ...
    addAgingValue(
        table, data.days90PlusBalance(), data.days90PlusBalance().compareTo(BigDecimal.ZERO) > 0);

    // Total with emphasis
    PdfPCell totalCell =
        new PdfPCell(
            new Phrase(
                formatCurrency(data.totalBalance()), new Font(Font.HELVETICA, 11, Font.BOLD)));
  ...
  private void addAgingValue(PdfPTable table, BigDecimal amount, boolean warning) {
    Font font = warning ? new Font(Font.HELVETICA, 10, Font.BOLD, WARNING_COLOR) : NORMAL_FONT;

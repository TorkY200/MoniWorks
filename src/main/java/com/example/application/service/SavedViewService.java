...
/**
 * Service for managing saved grid views per user.
 * Allows users to save, retrieve, and manage custom grid configurations.
 */
@Service
@Transactional
public class SavedViewService {
    ...
    public List<SavedView> findByEntityType(Company company, User user, EntityType entityType) {
        return savedViewRepository.findByCompanyAndUserAndEntityTypeOrderByName(company, user, entityType);
    }
     ...
     */
    @Transactional(readOnly = true)
    public Optional<SavedView> findByName(Company company, User user, EntityType entityType, String name) {
        return savedViewRepository.findByCompanyAndUserAndEntityTypeAndName(company, user, entityType, name);
    }
    ...
    public Optional<SavedView> findDefaultView(Company company, User user, EntityType entityType) {
        return savedViewRepository.findByCompanyAndUserAndEntityTypeAndIsDefaultTrue(company, user, entityType);
    }

    /**
     * Creates a new saved view.
     */
    public SavedView create(Company company, User user, EntityType entityType, String name,
            String columnsJson, String filtersJson, String sortJson) {
        if (savedViewRepository.existsByCompanyAndUserAndEntityTypeAndName(company, user, entityType, name)) {
        ...
        auditService.logEvent(company, user, "SAVED_VIEW_CREATED", "SAVED_VIEW", saved.getId(),
            "Created saved view: " + name + " for " + entityType);

        log.info("Created saved view {} for user {} on entity type {}", name, user.getEmail(), entityType);
        return saved;
    }
    ...
    public SavedView update(SavedView view, String columnsJson, String filtersJson, String sortJson) {
        ...
        SavedView saved = savedViewRepository.save(view);
        auditService.logEvent(view.getCompany(), view.getUser(), "SAVED_VIEW_UPDATED", "SAVED_VIEW", saved.getId(),
    ...
    /**
     * Sets a view as the default for its entity type.
     * Clears any previous default.
     */
    public SavedView setAsDefault(SavedView view) {
        // Clear existing defaults for this entity type
        List<SavedView> existingViews = savedViewRepository.findByCompanyAndUserAndEntityTypeOrderByName(
        ...
        auditService.logEvent(view.getCompany(), view.getUser(), "SAVED_VIEW_SET_DEFAULT", "SAVED_VIEW", saved.getId(),
    ...
    public SavedView rename(SavedView view, String newName) {
        ...
        auditService.logEvent(view.getCompany(), view.getUser(), "SAVED_VIEW_RENAMED", "SAVED_VIEW", saved.getId(),
    ...
    public void delete(SavedView view) {
        auditService.logEvent(view.getCompany(), view.getUser(), "SAVED_VIEW_DELETED", "SAVED_VIEW", view.getId(),
     ...
     */
    public SavedView saveOrUpdate(Company company, User user, EntityType entityType, String name,
            String columnsJson, String filtersJson, String sortJson) {

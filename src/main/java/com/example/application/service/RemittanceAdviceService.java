...
/**
 * Service for generating remittance advice PDFs. Creates one remittance advice per supplier in a
 * payment run.
 */
@Service
@Transactional
public class RemittanceAdviceService {
  ...
  /**
   * Generates a combined remittance advice PDF for all suppliers in a payment run. Each supplier
   * gets their own page in the PDF.
   *
   ...
   */
  public Attachment generateRemittanceAdvice(
      PaymentRun paymentRun, List<PaymentRunService.PaymentRunBill> runBills, User creator) {
    ...
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      ...
      Map<Contact, List<PaymentRunService.PaymentRunBill>> bySupplier =
          runBills.stream().collect(Collectors.groupingBy(rb -> rb.bill().getContact()));

      boolean firstPage = true;
      for (Map.Entry<Contact, List<PaymentRunService.PaymentRunBill>> entry :
          bySupplier.entrySet()) {
        if (!firstPage) {
          document.newPage();
        }
      ...
      log.info("Generated remittance advice PDF: {} ({} bytes)", filename, baos.size());
  ...
  private void addSupplierRemittance(
      Document document,
      Company company,
      PaymentRun paymentRun,
      Contact supplier,
      List<PaymentRunService.PaymentRunBill> bills)
    ...
    for (PaymentRunService.PaymentRunBill rb : bills) {
      ...
      addTableCell(
          billsTable, bill.getBillDate().format(dateFormatter), bgColor, Element.ALIGN_CENTER);
      addTableCell(
          billsTable, bill.getDueDate().format(dateFormatter), bgColor, Element.ALIGN_CENTER);
      addTableCell(
          billsTable,
          bill.getSupplierReference() != null ? bill.getSupplierReference() : "-",
          bgColor,
          Element.ALIGN_LEFT);

 ...
 *
 * <p>Handles: - Tax calculation based on tax codes - Tax-inclusive and tax-exclusive amounts -
 * Creating TaxLine records for ledger entries - Rounding rules (half-up to 2 decimal places)
 *
 * <p>GST/VAT Calculation: - Tax-exclusive: amount * rate = tax - Tax-inclusive: amount * rate / (1
 * + rate) = tax
 */
@Service
@Transactional
public class TaxCalculationService {
  ...
  public TaxCalculationService(
      TaxCodeRepository taxCodeRepository, TaxLineRepository taxLineRepository) {
    this.taxCodeRepository = taxCodeRepository;
    this.taxLineRepository = taxLineRepository;
  }

  /**
   * Calculates tax for a given amount and tax code. Assumes tax-exclusive amount (add tax on top).
   *
  ...
  public BigDecimal calculateTax(Company company, String taxCodeStr, BigDecimal amount) {
    ...
    return amount.multiply(taxCode.getRate()).setScale(2, RoundingMode.HALF_UP);
  }

  /**
   * Calculates tax for a tax-inclusive amount. Extracts the tax component from an amount that
   * already includes tax.
   *
   * <p>Formula: tax = amount * rate / (1 + rate)
   *
   * @param company The company (for tax code lookup)
   ...
   */
  public BigDecimal calculateTaxFromInclusive(
      Company company, String taxCodeStr, BigDecimal inclusiveAmount) {
    if (taxCodeStr == null || taxCodeStr.isBlank()) {
      return BigDecimal.ZERO;
    }
    ...
    return inclusiveAmount.multiply(rate).divide(divisor, 2, RoundingMode.HALF_UP);
  }

  /**
   * Extracts the taxable (tax-exclusive) amount from a tax-inclusive amount.
   *
   * <p>Formula: taxable = amount / (1 + rate)
   *
   ...
   */
  public BigDecimal extractTaxableAmount(
      Company company, String taxCodeStr, BigDecimal inclusiveAmount) {
    if (taxCodeStr == null || taxCodeStr.isBlank()) {
  ...
  /**
   * Creates TaxLine records for all ledger entries in a posted transaction. Called during the
   * posting process to record tax calculations.
   *
   * @param company The company
   * @param ledgerEntries The ledger entries created during posting
   * @return List of created TaxLine records
   */
  public List<TaxLine> createTaxLinesForLedgerEntries(
      Company company, List<LedgerEntry> ledgerEntries) {
    ...
    for (LedgerEntry entry : ledgerEntries) {
      ...
      TaxLine taxLine =
          new TaxLine(company, entry, taxCodeStr, taxCode.getRate(), taxableAmount, taxAmount);

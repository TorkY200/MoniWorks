 ...
 * Supports budget-vs-actual reporting by account, period, and department.
 */
@Service
@Transactional
public class BudgetService {
    ...
    public Budget createBudget(Company company, String name, Budget.BudgetType type, User actor) {
        ...
        auditService.logEvent(company, actor, "BUDGET_CREATE", "Budget",
            budget.getId(), "Created budget: " + name + " (Type " + type + ")");
    ...
    public Budget save(Budget budget, User actor) {
        ...
        if (!isNew) {
            auditService.logEvent(budget.getCompany(), actor, "BUDGET_UPDATE",
                "Budget", saved.getId(),
    ...
    public void deactivate(Budget budget, User actor) {
        ...
        auditService.logEvent(budget.getCompany(), actor, "BUDGET_DEACTIVATE",
            "Budget", budget.getId(),
     ...
     */
    public BudgetLine saveBudgetLine(Budget budget, Period period, Account account,
                                      Department department, BigDecimal amount) {
    ...
    public BigDecimal getBudgetAmountForAccount(Budget budget, Account account, Long fiscalYearId) {
        BigDecimal sum = budgetLineRepository.sumByBudgetAndAccountAndFiscalYear(
            budget, account, fiscalYearId);
     ...
     */
    @Transactional(readOnly = true)
    public BigDecimal getBudgetAmountForAccountAndDepartment(Budget budget, Account account,
                                                              Department department, Long fiscalYearId) {
        BigDecimal sum = budgetLineRepository.sumByBudgetAndAccountAndDepartmentAndFiscalYear(
    ...
    public void deleteAllLinesForBudget(Budget budget, User actor) {
        ...
        auditService.logEvent(budget.getCompany(), actor, "BUDGET_LINES_CLEARED",
            "Budget", budget.getId(),

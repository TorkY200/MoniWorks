...
/**
 * Service for managing statement runs (batch statement generation). Creates statements for multiple
...
public class StatementRunService {
  ...
  public StatementRunService(
      StatementRunRepository runRepository,
  ...
  public record StatementCriteria(
    ...
    public static StatementCriteria defaultCriteria() {
      return new StatementCriteria(
          BigDecimal.ZERO, null, List.of(), false, StatementService.StatementType.OPEN_ITEM, null);
    }

    public static StatementCriteria openItemCriteria(
      ...
      return new StatementCriteria(
          ...
          contactIds,
          includeZeroBalance,
          StatementService.StatementType.OPEN_ITEM,
          null);
    }

    public static StatementCriteria balanceForwardCriteria(
        LocalDate periodStart, List<Long> contactIds, boolean includeZeroBalance) {
      return new StatementCriteria(
          null,
          null,
          contactIds,
          includeZeroBalance,
          StatementService.StatementType.BALANCE_FORWARD,
          periodStart);
    }
  }
   ...
   */
  public StatementRun createRun(
      Company company, LocalDate asOfDate, StatementCriteria criteria, User createdBy) {
  ...
  /**
   * Processes a statement run, generating PDFs for all matching customers. Combines all statements
   * into a single PDF attachment.
   *
    ...
    try {
      ...
      if (criteria.statementType() == StatementService.StatementType.BALANCE_FORWARD) {
        ...
        LocalDate periodStart =
            criteria.periodStart() != null ? criteria.periodStart() : asOfDate.minusMonths(1);
        combinedPdf =
            generateCombinedBalanceForwardPdf(
                company, customersWithBalances, periodStart, asOfDate);
        filename =
            "BalanceForward_Statements_"
      ...
      log.info(
          "Statement run {} completed: {} statements generated",
          run.getId(),
          customersWithBalances.size());
  ...
  public StatementRun createAndProcessRun(
      Company company, LocalDate asOfDate, StatementCriteria criteria, User user) {
    StatementRun run = createRun(company, asOfDate, criteria, user);
    return processRun(run, user);
  }

  /** Gets a single customer's statement PDF. Convenience method that wraps StatementService. */
  ...
  private List<Contact> findCustomersWithBalances(
      Company company, LocalDate asOfDate, StatementCriteria criteria) {
  ...
  private boolean hasOutstandingBalance(
      Company company, Contact contact, LocalDate asOfDate, StatementCriteria criteria) {
    StatementService.StatementData data =
        statementService.generateStatementData(company, contact, asOfDate);
    ...
    if (criteria.minimumBalance() != null
        && totalBalance.compareTo(criteria.minimumBalance()) < 0) {
      return false;
    }
  ...
  /**
   * Generates a combined PDF containing statements for multiple customers. Each customer starts on
   * a new page.
   */
  private byte[] generateCombinedStatementsPdf(
      Company company, List<Contact> customers, LocalDate asOfDate) {
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      ...
      for (Contact customer : customers) {
        ...
        StatementService.StatementData data =
            statementService.generateStatementData(company, customer, asOfDate);
      ...
      throw new RuntimeException(
          "Failed to generate combined statements PDF: " + e.getMessage(), e);
    }
  }

  /**
   * Generates a combined PDF containing balance-forward statements for multiple customers. Each
   * customer starts on a new page. Shows activity over a period with opening/closing balances (spec
   * 09).
   */
  private byte[] generateCombinedBalanceForwardPdf(
      Company company, List<Contact> customers, LocalDate periodStart, LocalDate periodEnd) {
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      Document document = new Document(PageSize.A4, 50, 50, 50, 50);
      PdfWriter writer = PdfWriter.getInstance(document, baos);
      document.open();
      ...
      for (Contact customer : customers) {
        ...
        StatementService.BalanceForwardStatementData data =
            statementService.generateBalanceForwardStatementData(
                company, customer, periodStart, periodEnd);
      ...
      throw new RuntimeException(
          "Failed to generate combined balance-forward statements PDF: " + e.getMessage(), e);
    }
  }
  ...
  private StatementCriteria parseCriteria(String json) {
    ...
    try {
      ...
      if (node.has("statementType")) {
        try {
          statementType =
              StatementService.StatementType.valueOf(node.get("statementType").asText());
  ...
  @Transactional(readOnly = true)
  public List<StatementRun> findByDateRange(
  ...
  /**
   * Gets customers who would be included in a statement run with the given criteria. Useful for
   * previewing before running.
   */
  @Transactional(readOnly = true)
  public List<Contact> previewCustomers(
      Company company, LocalDate asOfDate, StatementCriteria criteria) {

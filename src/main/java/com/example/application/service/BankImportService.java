...
/**
 * Service for importing bank statements from various formats.
 * Supports QIF (Quicken Interchange Format) and OFX (Open Financial Exchange).
 */
@Service
@Transactional
public class BankImportService {
    ...
    public BankImportService(BankStatementImportRepository importRepository,
    ...
    /**
     * Imports a bank statement file for the given bank account.
     * Returns the import record with statistics.
     *
     ...
     */
    public BankStatementImport importStatement(Company company, Account bankAccount,
                                                String fileName, String content) {
        if (!bankAccount.isBankAccount()) {
            throw new IllegalArgumentException("Account is not marked as a bank account: " +
                bankAccount.getCode());
        }
        ...
        BankStatementImport importRecord = new BankStatementImport(
            company, bankAccount, sourceType, fileName, fileHash);
        ...
        for (BankFeedItem item : items) {
            if (item.getFitId() != null &&
                feedItemRepository.existsByBankStatementImportAndFitId(importRecord, item.getFitId())) {
                duplicateCount++;
                continue;
            }
            newItems.add(item);
        ...
        log.info("Imported {} items from {} ({} duplicates skipped)",
            newItems.size(), fileName, duplicateCount);
     ...
     */
    private List<BankFeedItem> parseStatement(BankStatementImport importRecord,
                                               String content, SourceType sourceType) {
     ...
     * QIF format uses single-character field codes:
     * D = Date, T = Amount, P = Payee, M = Memo, N = Number/Reference
     */
            ...
            switch (code) {
                ...
                case '^' -> {
                    ...
                    if (currentDate != null && currentAmount != null) {
                        ...
                        BankFeedItem item = new BankFeedItem(
                            importRecord, currentDate, currentAmount, description);
                        item.setFitId(currentNumber);
                        item.setRawJson(buildRawJson(currentDate, currentAmount,
                            currentPayee, currentMemo, currentNumber));
                        items.add(item);
                    }
    ...
    /**
     * Parses OFX/QFX/QBO (Open Financial Exchange) files.
     * Basic SGML-style parser for common OFX elements.
     */
    private List<BankFeedItem> parseOFX(BankStatementImport importRecord, String content) {
        ...
        while ((pos = content.indexOf("<STMTTRN>", pos)) != -1) {
            ...
            if (date != null && amount != null) {
                ...
                BankFeedItem item = new BankFeedItem(
                    importRecord, date, amount, description);
                item.setFitId(fitId);
    ...
    /**
     * Parses CSV files with common bank export formats.
     * Expects headers: Date, Description/Payee, Amount (or Debit/Credit columns)
     */
    private List<BankFeedItem> parseCSV(BankStatementImport importRecord, String content) {
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            if (amountCol >= 0 && amountCol < values.length) {
                ...
                BigDecimal debit = debitCol >= 0 && debitCol < values.length ?
                    parseAmount(values[debitCol]) : BigDecimal.ZERO;
                BigDecimal credit = creditCol >= 0 && creditCol < values.length ?
                    parseAmount(values[creditCol]) : BigDecimal.ZERO;
                amount = credit.subtract(debit);
            }

            if (amount != null && amount.compareTo(BigDecimal.ZERO) != 0) {
                String description = descCol >= 0 && descCol < values.length ?
                    values[descCol].trim() : "";
                BankFeedItem item = new BankFeedItem(
                    importRecord, date, amount, description);
    ...
    private String buildRawJson(LocalDate date, BigDecimal amount,
                                 String payee, String memo, String number) {

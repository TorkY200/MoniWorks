...
public class PayableAllocationService {
  ...
  public PayableAllocationService(
      PayableAllocationRepository allocationRepository,
  ...
  /**
   * Allocates a payment to a bill. Updates the bill's amount paid.
   *
   ...
   */
  public PayableAllocation allocate(
      Transaction payment, SupplierBill bill, BigDecimal amount, User allocatedBy) {
    validateAllocation(payment, bill, amount);

    Company company = bill.getCompany();

    PayableAllocation allocation = new PayableAllocation(company, payment, bill, amount);
   ...
   */
  public List<PayableAllocation> allocateToMultiple(
      Transaction payment, List<AllocationRequest> allocations, User allocatedBy) {
    List<PayableAllocation> created = new ArrayList<>();

    for (AllocationRequest request : allocations) {
      PayableAllocation allocation =
          allocate(payment, request.bill(), request.amount(), allocatedBy);
  ...
  /** Removes an allocation. Updates the bill's amount paid. */
  public void removeAllocation(PayableAllocation allocation, User actor) {
    ...
    auditService.logEvent(
        ...
        "PayableAllocation",
        allocation.getId(),
        "Removed allocation from bill " + bill.getBillNumber());
  }

  /**
   * Suggests allocations for a payment based on outstanding bills. Uses a simple matching strategy:
   * 1. Exact amount match to a single bill 2. Oldest bills first until payment amount is exhausted
   *
  ...
  public List<AllocationSuggestion> suggestAllocations(
      Company company, Contact contact, BigDecimal paymentAmount) {
  ...
  private void validateAllocation(Transaction payment, SupplierBill bill, BigDecimal amount) {
    ...
    if (amount.compareTo(unallocated) > 0) {
      throw new IllegalArgumentException(
          "Allocation amount ($"
              + amount
              + ") exceeds unallocated payment amount ($"
              + unallocated
              + ")");
    }
  }
  ...
  public record AllocationSuggestion(
      SupplierBill bill, BigDecimal suggestedAmount, boolean exactMatch) {}
}

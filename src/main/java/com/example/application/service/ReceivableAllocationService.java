...
public class ReceivableAllocationService {
  ...
  public ReceivableAllocationService(
  ...
  /**
   * Allocates a receipt to an invoice. Updates the invoice's amount paid.
   *
  ...
  public ReceivableAllocation allocate(
      Transaction receipt, SalesInvoice invoice, BigDecimal amount, User allocatedBy) {
    validateAllocation(receipt, invoice, amount);

    Company company = invoice.getCompany();

    ReceivableAllocation allocation = new ReceivableAllocation(company, receipt, invoice, amount);
   ...
   */
  public List<ReceivableAllocation> allocateToMultiple(
      Transaction receipt, List<AllocationRequest> allocations, User allocatedBy) {
    List<ReceivableAllocation> created = new ArrayList<>();

    for (AllocationRequest request : allocations) {
      ReceivableAllocation allocation =
          allocate(receipt, request.invoice(), request.amount(), allocatedBy);
  ...
  /** Removes an allocation. Updates the invoice's amount paid. */
  public void removeAllocation(ReceivableAllocation allocation, User actor) {
    ...
    auditService.logEvent(
        ...
        "ReceivableAllocation",
        allocation.getId(),
        "Removed allocation from invoice " + invoice.getInvoiceNumber());
  }

  /**
   * Suggests allocations for a receipt based on outstanding invoices. Uses a simple matching
   * strategy: 1. Exact amount match to a single invoice 2. Oldest invoices first until receipt
   * amount is exhausted
   *
  ...
  public List<AllocationSuggestion> suggestAllocations(
      Company company, Contact contact, BigDecimal receiptAmount) {
  ...
  private void validateAllocation(Transaction receipt, SalesInvoice invoice, BigDecimal amount) {
    ...
    if (amount.compareTo(unallocated) > 0) {
      throw new IllegalArgumentException(
          "Allocation amount ($"
              + amount
              + ") exceeds unallocated receipt amount ($"
              + unallocated
              + ")");
    }
  }
  ...
  public record AllocationSuggestion(
      SalesInvoice invoice, BigDecimal suggestedAmount, boolean exactMatch) {}
}

...
/**
 * Service for managing user invitations. Handles the complete invitation workflow: create, send,
...
public class InvitationService {
  ...
  public record InvitationResult(
      boolean success, String message, UserInvitation invitation, User user) {
  ...
  public InvitationResult createInvitation(
      String email, String displayName, Company company, Role role, User invitedBy) {
    // Normalize email
    ...
    UserInvitation invitation =
        new UserInvitation(normalizedEmail, token, company, role, expiresAt);
    ...
    log.info(
        "Invitation created for {} to join company {} with role {}",
        normalizedEmail,
        company.getName(),
        role.getName());
   ...
   */
  public InvitationResult acceptInvitationNewUser(
    ...
    if (userRepository.findByEmail(invitation.getEmail()).isPresent()) {
      return InvitationResult.failure(
          "An account with this email already exists. Please use 'Accept for Existing User'.");
    }
    ...
    CompanyMembership membership =
        new CompanyMembership(user, invitation.getCompany(), invitation.getRole());
    ...
    log.info(
        "Invitation accepted: new user {} joined company {} with role {}",
        invitation.getEmail(),
  ...
  public InvitationResult acceptInvitationExistingUser(String token, User user) {
    ...
    log.info(
        "Invitation accepted: existing user {} joined company {} with role {}",
        user.getEmail(),
        invitation.getCompany().getName(),
        invitation.getRole().getName());
  ...
  public List<UserInvitation> getPendingInvitationsByCompany(Company company) {
    return invitationRepository.findByCompanyAndStatusOrderByCreatedAtDesc(
  ...
  /** Scheduled task to expire old invitations. Runs daily at 3 AM. */
  @Scheduled(cron = "0 0 3 * * *")
  public void expireOldInvitations() {
    List<UserInvitation> expired =
        invitationRepository.findExpiredPendingInvitations(Instant.now());
  ...
  private void sendInvitationEmail(UserInvitation invitation) {
    ...
    String subject =
        String.format("You've been invited to join %s on MoniWorks", company.getName());
    ...
    if (!result.success()) {
      log.warn(
          "Failed to send invitation email to {}: {}", invitation.getEmail(), result.message());
    }
  }
}

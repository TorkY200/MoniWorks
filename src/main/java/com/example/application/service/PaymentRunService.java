 ...
 *
 * <p>Key workflows: 1. Create payment run → Select bills to pay → Complete run (creates payment
 * transactions) 2. Group payments by supplier for efficient bank transfers
 *
 * <p>Completing a payment run: - Creates payment transactions for each supplier - Allocates
 * payments to bills - Generates remittance advice (PDF output)
 */
@Service
@Transactional
public class PaymentRunService {
  ...
  public PaymentRun createPaymentRun(
      Company company, LocalDate runDate, Account bankAccount, User createdBy) {
    ...
    auditService.logEvent(
        ...
        "PaymentRun",
        run.getId(),
        "Created payment run for " + runDate);

    return run;
  }

  /** Adds bills to a payment run. Bills are grouped by supplier for efficient processing. */
    ...
    for (SupplierBill bill : bills) {
      ...
      if (bill.getBalance().compareTo(BigDecimal.ZERO) <= 0) {
        throw new IllegalArgumentException(
            "Bill " + bill.getBillNumber() + " has no outstanding balance");
      }
  ...
  /**
   * Completes a payment run. Creates payment transactions grouped by supplier and allocates to
   * bills.
   */
  public PaymentRun completePaymentRun(PaymentRun run, User actor) {
    ...
    Map<Long, List<PaymentItem>> bySupplier =
        items.stream().collect(Collectors.groupingBy(item -> item.contactId));
    ...
    for (Map.Entry<Long, List<PaymentItem>> entry : bySupplier.entrySet()) {
      ...
      BigDecimal totalAmount =
          supplierItems.stream().map(item -> item.amount).reduce(BigDecimal.ZERO, BigDecimal::add);

      // Get first bill to get contact
      SupplierBill firstBill =
          billRepository
              .findById(supplierItems.get(0).billId)
              .orElseThrow(() -> new IllegalStateException("Bill not found"));
      Contact supplier = firstBill.getContact();

      // Create payment transaction
      String description =
          "Payment to " + supplier.getName() + " (" + supplierItems.size() + " bills)";
      Transaction payment =
          transactionService.createTransaction(
              company, Transaction.TransactionType.PAYMENT, run.getRunDate(), description, actor);

      // Credit bank account
      TransactionLine bankLine =
          new TransactionLine(bankAccount, totalAmount, TransactionLine.Direction.CREDIT);
      ...
      Account apAccount =
          accountService
              ...
              .orElseThrow(
                  () -> new IllegalStateException("AP account not found: " + AP_ACCOUNT_CODE));

      TransactionLine apLine =
          new TransactionLine(apAccount, totalAmount, TransactionLine.Direction.DEBIT);
    ...
    Attachment remittancePdf =
        remittanceAdviceService.generateRemittanceAdvice(run, runBills, actor);
    ...
    auditService.logEvent(
        company,
        actor,
        "PAYMENT_RUN_COMPLETED",
        "PaymentRun",
        run.getId(),
        "Completed payment run with " + items.size() + " bills, generated remittance advice");
  ...
  public BigDecimal getRunTotal(PaymentRun run) {
    List<PaymentItem> items = getPaymentItems(run);
    return items.stream().map(item -> item.amount).reduce(BigDecimal.ZERO, BigDecimal::add);
  }
  ...
  private List<PaymentItem> getPaymentItems(PaymentRun run) {
    ...
    try {
      return objectMapper.readValue(run.getItemsJson(), new TypeReference<List<PaymentItem>>() {});

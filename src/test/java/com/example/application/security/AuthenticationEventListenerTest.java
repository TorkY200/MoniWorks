...
/**
 * Unit tests for AuthenticationEventListener. Verifies that login success and failure events are
 * properly logged to the audit trail.
 */
...
class AuthenticationEventListenerTest {
  ...
  void onAuthenticationSuccess_userNotFound_doesNotLog() {
    ...
    UserDetails userDetails =
        new org.springframework.security.core.userdetails.User(
            "unknown@example.com", "password", Collections.emptyList());
    UsernamePasswordAuthenticationToken authentication =
        new UsernamePasswordAuthenticationToken(userDetails, null, Collections.emptyList());
    AuthenticationSuccessEvent event = new AuthenticationSuccessEvent(authentication);
  ...
  void onAuthenticationFailure_logsFailedAttempt() {
    ...
    when(auditService.logEvent(
            any(), any(), eq("LOGIN_FAILED"), eq("User"), any(), anyString(), anyMap()))

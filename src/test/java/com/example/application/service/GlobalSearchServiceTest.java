...
class GlobalSearchServiceTest {
  ...
  void parseQuery_mixedQuery_extractsBoth() {
    GlobalSearchService.ParsedQuery result =
        globalSearchService.parseQuery("type:invoice status:overdue acme");
  ...
  void parseQuery_complexQuery_extractsAll() {
    GlobalSearchService.ParsedQuery result =
        globalSearchService.parseQuery("type:bill amount>500 older_than:60d supplier");
  ...
  void search_withTypeFilter_searchesOnlyMatchingType() {
    ...
    verify(salesInvoiceRepository, never())
        .findByCompanyOrderByIssueDateDescInvoiceNumberDesc(any());
  }
  ...
  void search_withFreeText_filtersResults() {
    ...
    when(transactionRepository.findByCompanyOrderByTransactionDateDesc(company))
        .thenReturn(List.of());
  ...
  void search_withStatusFilter_filtersContactsByActiveStatus() {
    Contact activeContact =
        new Contact(company, "ACT01", "Active Co", Contact.ContactType.CUSTOMER);
    activeContact.setId(1L);
    activeContact.setActive(true);

    Contact inactiveContact =
        new Contact(company, "INA01", "Inactive Co", Contact.ContactType.CUSTOMER);
    inactiveContact.setId(2L);
    inactiveContact.setActive(false);

    when(contactRepository.findByCompanyOrderByCode(company))
        .thenReturn(List.of(activeContact, inactiveContact));
    when(transactionRepository.findByCompanyOrderByTransactionDateDesc(company))
        .thenReturn(List.of());
    when(productRepository.findByCompanyOrderByCode(company)).thenReturn(List.of());
    when(accountRepository.findByCompanyOrderByCode(company)).thenReturn(List.of());
    when(salesInvoiceRepository.findByCompanyOrderByIssueDateDescInvoiceNumberDesc(company))
        .thenReturn(List.of());
    when(supplierBillRepository.findByCompanyOrderByBillDateDescBillNumberDesc(company))
        .thenReturn(List.of());

    List<GlobalSearchResult> results = globalSearchService.search(company, "status:active");

    // Filter to only contact results
    List<GlobalSearchResult> contactResults =
        results.stream()

...
/**
 * Unit tests for TaxCalculationService. Tests tax calculation logic for GST/VAT including: -
 * Tax-exclusive calculations (add tax to amount) - Tax-inclusive calculations (extract tax from
 * amount) - Different tax types (standard, zero-rated, exempt, out of scope) - Rounding rules
...
class TaxCalculationServiceTest {
  ...
  void setUp() {
    ...
    // GST 15%
    gstCode =
        new TaxCode(company, "GST", "GST 15%", new BigDecimal("0.15"), TaxCode.TaxType.STANDARD);
    gstCode.setReportBox("5");

    // Zero rated
    zeroCode =
        new TaxCode(company, "ZERO", "Zero Rated", BigDecimal.ZERO, TaxCode.TaxType.ZERO_RATED);

    // Exempt
    exemptCode = new TaxCode(company, "EXEMPT", "Exempt", BigDecimal.ZERO, TaxCode.TaxType.EXEMPT);
  }

  @Test
  void calculateTax_standardRate_calculatesCorrectly() {
    // NZ GST: 15% of 100.00 = 15.00
    when(taxCodeRepository.findByCompanyAndCode(company, "GST")).thenReturn(Optional.of(gstCode));

    BigDecimal tax = taxCalculationService.calculateTax(company, "GST", new BigDecimal("100.00"));
  ...
  void calculateTax_withRounding_roundsHalfUp() {
    ...
    BigDecimal tax = taxCalculationService.calculateTax(company, "GST", new BigDecimal("33.33"));
  ...
  void calculateTax_zeroRated_returnsZero() {
    when(taxCodeRepository.findByCompanyAndCode(company, "ZERO")).thenReturn(Optional.of(zeroCode));

    BigDecimal tax = taxCalculationService.calculateTax(company, "ZERO", new BigDecimal("100.00"));
  ...
  void calculateTax_exempt_returnsZero() {
    ...
    BigDecimal tax =
        taxCalculationService.calculateTax(company, "EXEMPT", new BigDecimal("100.00"));
  ...
  void calculateTax_nullTaxCode_returnsZero() {
    BigDecimal tax = taxCalculationService.calculateTax(company, null, new BigDecimal("100.00"));
  ...
  void calculateTax_unknownTaxCode_returnsZero() {
    when(taxCodeRepository.findByCompanyAndCode(company, "UNKNOWN")).thenReturn(Optional.empty());

    BigDecimal tax =
        taxCalculationService.calculateTax(company, "UNKNOWN", new BigDecimal("100.00"));
  ...
  void calculateTaxFromInclusive_standardRate_extractsCorrectly() {
    ...
    BigDecimal tax =
        taxCalculationService.calculateTaxFromInclusive(company, "GST", new BigDecimal("115.00"));
  ...
  void calculateTaxFromInclusive_withRounding_roundsHalfUp() {
    ...
    BigDecimal tax =
        taxCalculationService.calculateTaxFromInclusive(company, "GST", new BigDecimal("100.00"));
  ...
  void extractTaxableAmount_standardRate_extractsCorrectly() {
    ...
    BigDecimal taxable =
        taxCalculationService.extractTaxableAmount(company, "GST", new BigDecimal("115.00"));
  ...
  void extractTaxableAmount_exempt_returnsFullAmount() {
    ...
    BigDecimal taxable =
        taxCalculationService.extractTaxableAmount(company, "EXEMPT", new BigDecimal("100.00"));

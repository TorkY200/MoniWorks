...
class ProductImportServiceTest {
  ...
  void importProducts_ValidCsv_ImportsAllProducts() throws IOException {
    ...
    when(productRepository.findByCompanyAndCode(any(), anyString())).thenReturn(Optional.empty());
    when(productRepository.save(any(Product.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));

    // When
    ImportResult result = importService.importProducts(toInputStream(csv), company, user, false);

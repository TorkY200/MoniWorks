...
/**
 * Unit tests for InvitationService. Tests invitation creation, acceptance, cancellation, and
...
class InvitationServiceTest {
  ...
  void createInvitation_NewEmail_Success() {
    ...
    InvitationResult result =
        invitationService.createInvitation(email, "New User", company, role, inviter);
  ...
  void createInvitation_UserAlreadyMember_Fails() {
    ...
    // When
    InvitationResult result =
        invitationService.createInvitation(email, null, company, role, inviter);
  ...
  void acceptInvitationNewUser_ValidInvitation_CreatesUserAndMembership() {
    ...
    // When
    InvitationResult result =
        invitationService.acceptInvitationNewUser("valid-token", "New User", "password123");
  ...
  void acceptInvitationNewUser_EmailAlreadyRegistered_Fails() {
    ...
    // When
    InvitationResult result =
        invitationService.acceptInvitationNewUser("valid-token", "Name", "password");
  ...
  void acceptInvitationNewUser_ExpiredInvitation_Fails() {
    ...
    // When
    InvitationResult result =
        invitationService.acceptInvitationNewUser("expired-token", "Name", "password");
  ...
  void acceptInvitationExistingUser_ValidInvitation_CreatesMembership() {
    ...
    // When
    InvitationResult result =
        invitationService.acceptInvitationExistingUser("valid-token", existingUser);
  ...
  void cancelInvitation_PendingInvitation_Success() {
    ...
    verify(invitationRepository)
        .save(argThat(inv -> inv.getStatus() == InvitationStatus.CANCELLED));
    verify(auditService)
        .logEvent(
            eq(company),
            eq(inviter),
            eq("INVITATION_CANCELLED"),
            eq("USER_INVITATION"),
            anyLong(),
            anyString());
  }

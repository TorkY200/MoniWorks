...
class BudgetImportServiceTest {
  ...
  void importBudgetLines_ValidCsv_ImportsAllLines() throws IOException {
    ...
    when(budgetLineRepository.findByBudgetAndPeriodAndAccountAndDepartment(
            any(), any(), any(), any()))
    ...
    // When
    ImportResult result = importService.importBudgetLines(toInputStream(csv), budget, user, false);
    ...
    verify(auditService)
        .logEvent(
            eq(company), eq(user), eq("BUDGET_IMPORTED"), eq("Budget"), eq(1L), contains("2 new"));
  }
  ...
  void importBudgetLines_ExistingLine_SkipsWithoutUpdate() throws IOException {
    ...
    when(periodRepository.findByCompanyAndDate(company, LocalDate.of(2024, 7, 1)))
        .thenReturn(Optional.of(period));
    when(budgetLineRepository.findByBudgetAndPeriodAndAccountAndDepartment(
            budget, period, incomeAccount, null))
        .thenReturn(Optional.of(existing));

    // When
    ImportResult result =
        importService.importBudgetLines(
  ...
  void importBudgetLines_WithDepartment_ParsesCorrectly() throws IOException {
    ...
    when(budgetLineRepository.findByBudgetAndPeriodAndAccountAndDepartment(
            budget, period, incomeAccount, salesDept))
        .thenReturn(Optional.empty());
    when(budgetLineRepository.save(any(BudgetLine.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
  ...
  void importBudgetLines_DepartmentNotFound_ReportsError() throws IOException {
    ...
    assertFalse(result.success());
    assertTrue(
        result.errors().stream().anyMatch(e -> e.contains("Department 'INVALID' not found")));
  }
  ...
  void previewImport_DoesNotSaveAnything() throws IOException {
    ...
    // When
    ImportResult result = importService.previewImport(toInputStream(csv), budget, false);

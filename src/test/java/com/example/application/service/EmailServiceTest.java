 ...
 * Tests email validation, result handling, and specialized email methods.
 */
...
class EmailServiceTest {
    ...
    void sendEmail_ValidRequest_WhenEnabled_ReturnsQueuedStatus() {
        ...
        verify(auditService).logEvent(
            eq(company), eq(user), eq("EMAIL_SENT"),
            eq("EMAIL"), isNull(), contains("recipient@example.com")
        );
    }
    ...
    void sendEmail_WithAttachments_LogsAttachmentCount() {
        ...
        EmailAttachment attachment = new EmailAttachment(
            "document.pdf", "application/pdf", "content".getBytes()
        );
    ...
    void sendReport_ValidRequest_SendsEmail() {
        ...
        // When
        EmailResult result = emailService.sendReport(

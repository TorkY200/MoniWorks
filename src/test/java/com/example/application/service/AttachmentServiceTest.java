...
/**
 * Unit tests for AttachmentService. Tests file upload, storage, linking, and retrieval
 * functionality.
 */
...
class AttachmentServiceTest {
  ...
  @Mock private AttachmentLinkRepository linkRepository;
  ...
  void uploadFile_AllowedImageTypes_Succeeds() {
    ...
    for (String mimeType : allowedTypes) {
      // When
      Attachment result =
          attachmentService.uploadFile(company, "image.test", mimeType, content, user);
  ...
  void linkToEntity_CreatesLink() {
    ...
    when(linkRepository.findByAttachmentAndEntityTypeAndEntityId(any(), any(), anyLong()))
        .thenReturn(Optional.empty());
    when(linkRepository.save(any(AttachmentLink.class)))
        .thenAnswer(
            invocation -> {
    ...
    AttachmentLink result =
        attachmentService.linkToEntity(attachment, EntityType.TRANSACTION, 100L);
  ...
  void linkToEntity_ExistingLink_ReturnsExisting() {
    ...
    when(linkRepository.findByAttachmentAndEntityTypeAndEntityId(
            attachment, EntityType.TRANSACTION, 100L))
  ...
  void getFileContent_ValidFile_ReturnsContent() throws Exception {
    ...
    Attachment attachment =
        attachmentService.uploadFile(company, filename, mimeType, originalContent, user);
  ...
  void deleteAttachment_RemovesFileAndRecord() throws Exception {
    ...
    verify(auditService)
        .logEvent(
            eq(company),
            eq(user),
            eq("ATTACHMENT_DELETED"),
            eq("ATTACHMENT"),
            eq(1L),
            contains("to-delete.pdf"));
  }
  ...
  void checksumCalculation_SameContent_SameChecksum() {
    ...
    // When
    Attachment att1 =
        attachmentService.uploadFile(company, "file1.pdf", "application/pdf", content, user);
    ...
    Attachment att2 =
        attachmentService.uploadFile(company, "file2.pdf", "application/pdf", content, user);
